...
import { isClaude4PlusModelFamily, isGPT5ModelFamily, isLocalModel, isNextGenModelFamily, isParallelToolCallingEnabled, } from "@utils/model-utils";
...
import { CommandExecutor, StandaloneTerminalManager, } from "@/integrations/terminal";
...
export class Task {
    ...
    constructor(params) {
            ...
            onRetryAttempt: async (attempt, maxRetries, delay, error) => {
                ...
                if (lastApiReqStartedIndex !== -1) {
                    try {
                        ...
                        await this.postStateToWebview().catch((e) => Logger.error("Error posting state to webview in onRetryAttempt:", e));
                    }
                    catch (e) {
        ...
        const commandExecutorCallbacks = {
            ...
            ask: async (type, text, partial) => {
                const result = await this.ask(type, text, partial);
                return {
            ...
            updateBackgroundCommandState: (isRunning) => this.controller.updateBackgroundCommandState(isRunning, this.taskId),
     ...
     */
    calculatePreCompactDeletedRange(apiConversationHistory) {
        const newDeletedRange = this.contextManager.getNextTruncationRange(apiConversationHistory, this.taskState.conversationHistoryDeletedRange, "quarter");
    ...
    // Tools
    async executeCommandTool(command, timeoutSeconds, options) {
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const promptContext = {
            ...
            clineWebToolsEnabled: this.stateManager.getGlobalSettingsKey("clineWebToolsEnabled") && featureFlagsService.getWebtoolsEnabled(),
            isMultiRootEnabled: multiRootEnabled,
            workspaceRoots,
            isSubagentRun: false,
            isCliEnvironment,
            enableNativeToolCalls: providerInfo.model.info.apiFormat === ApiFormat.OPENAI_RESPONSES ||
        ...
        try {
            ...
            if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
                ...
                if (!isClineProviderInsufficientCredits && !isAuthError && this.taskState.autoRetryAttempts < 3) {
                    ...
                    const autoRetryApiReqIndex = findLastIndex(this.messageStateHandler.getClineMessages(), (m) => m.say === "api_req_started");
                    if (autoRetryApiReqIndex !== -1) {
                        const clineMessages = this.messageStateHandler.getClineMessages();
                        const currentApiReqInfo = JSON.parse(clineMessages[autoRetryApiReqIndex].text || "{}");
                        delete currentApiReqInfo.streamingFailedMessage;
        ...
        switch (block.type) {
            ...
            case "tool_use":
                ...
                if (this.initialCheckpointCommitPromise) {
                    if (!READ_ONLY_TOOLS.includes(block.name)) {
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
            ...
            if (this.stateManager.getGlobalSettingsKey("yoloModeToggled")) {
                const errorMessage = `[YOLO MODE] Task failed: Too many consecutive mistakes (${this.taskState.consecutiveMistakeCount}). ` +
        ...
        if (useAutoCondense && isNextGenModelFamily(this.api.getModel().id)) {
            ...
            if (this.taskState.currentlySummarizing) {
                ...
                shouldCompact = this.contextManager.shouldCompactContextWindow(this.messageStateHandler.getClineMessages(), this.api, previousApiReqIndex);
        ...
        if (shouldCompact) {
            ...
            ;
            [parsedUserContent, environmentDetails, clinerulesError] = await this.loadContext(userContent, includeFileDetails, useCompactPrompt);
        }
        ...
        await this.postStateToWebview();
        try {
            const taskMetrics = { cacheWriteTokens: 0, cacheReadTokens: 0, inputTokens: 0, outputTokens: 0, totalCost: undefined };
            ...
            */
            const updateApiReqMsgFromMetrics = async (cancelReason, streamingFailedMessage) => {
                await updateApiReqMsg({
                    messageStateHandler: this.messageStateHandler,
                    lastApiReqIndex,
            ...
            const queueUsageChunkSideEffects = (usageInputTokens, usageOutputTokens, chunkOptions) => {
                usageChunkSideEffectsQueue = usageChunkSideEffectsQueue
                    ...
                    await this.postStateToWebview();
                    await telemetryService.captureTokenUsage(this.ulid, usageInputTokens, usageOutputTokens, providerId, model.id, chunkOptions);
                })
                    .catch((error) => {
                    Logger.debug(`[Task ${this.taskId}] Failed to process usage chunk side effects: ${error}`);
                });
            };
            const finalizeApiReqMsg = async (cancelReason, streamingFailedMessage) => {
            ...
            const abortStream = async (cancelReason, streamingFailedMessage) => {
                ...
                telemetryService.captureConversationTurnEvent(this.ulid, providerId, modelInfo.modelId, "assistant", modelInfo.mode, undefined, this.useNativeToolCalls);
            ...
            const finalizePendingReasoningMessage = async (thinking) => {
                const pendingReasoningIndex = findLastIndex(this.messageStateHandler.getClineMessages(), (message) => message.type === "say" && message.say === "reasoning" && message.partial === true);
            ...
            if (assistantHasContent) {
                telemetryService.captureConversationTurnEvent(this.ulid, providerId, model.id, "assistant", modelInfo.mode, undefined, this.useNativeToolCalls);
    ...
    async loadContext(userContent, includeFileDetails = false, useCompactPrompt = false) {
        ...
        const parseTextBlock = async (text) => {
            ...
            const mcpPromptFetcher = async (serverName, promptName) => {
                try {
            ...
            const { processedText, needsClinerulesFileCheck: needsCheck } = await parseSlashCommands(parsedText, localWorkflowToggles, globalWorkflowToggles, ulid, focusChainSettings, useNativeToolCalls, providerInfo, mcpPromptFetcher);
            if (needsCheck) {
    ...
    async processNativeToolCalls(assistantTextOnly, toolBlocks) {

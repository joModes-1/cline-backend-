    ...
    describe("runPreToolUseIfEnabled", () => {
        it("returns early without running hooks when hooks are disabled", async () => {
            ...
            const config = {
                ...
                services: {
                    stateManager: {
                        getGlobalSettingsKey: (key) => (key === "hooksEnabled" ? false : undefined),
                    },
                },
                callbacks: {
                    say: saySpy,
        ...
        it("treats undefined hooksEnabled as enabled and runs hook flow", async () => {
            ...
            const getGlobalSettingsKeySpy = sinon.spy((key) => (key === "mode" ? "act" : undefined));
...
//# sourceMappingURL=ToolHookUtils.test.js.map
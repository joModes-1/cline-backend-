...
export class ApplyPatchHandler {
    ...
    async patchToCommit(patch, originalFiles) {
        const changes = {};
     ...
     */
    async prepareFileChange(change, path) {
     ...
     */
    async saveFileChange(change, path) {
        const ops = this.providerOps;
        switch (change.type) {
            case PatchActionType.DELETE:
    ...
    async handleApproval(config, block, message, rawInput, change) {

...
export class WriteToFileToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			const autoApproved = await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)
			if (autoApproved) {
				// Auto-approval flow
			...
			if (finalContent === undefined) {
				throw new Error(
					`File save did not persist for ${relPath}: saveChanges() returned undefined finalContent. ` +
			...
			Logger.info(
				`[WriteToFileTool] ✓ Saved ${block.name} for ${relPath} (absolute=${absolutePath}, bytes=${finalContent.length})`,
			)
	...
	async validateAndPrepareFileOperation(config: TaskConfig, block: ToolUse, relPath: string, diff?: string, content?: string) {
		...
		if (diff) {
			...
			try {
				...
				Logger.warn(
					`[WriteToFileTool] ✗ replace_in_file SEARCH did not match ${relPath}: ${
						(error as Error)?.message?.split("\n")[0] ?? "unknown"
					}. File will be reverted; LLM will be asked to retry.`,
				)

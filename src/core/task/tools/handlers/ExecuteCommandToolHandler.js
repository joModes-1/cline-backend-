...
export function resolveCommandTimeoutSeconds(command, timeoutParam, useManagedTimeout) {
...
export class ExecuteCommandToolHandler {
    ...
    async execute(config, block) {
        ...
        timeoutSeconds = resolveCommandTimeoutSeconds(command, timeoutParam, config.yoloModeToggled || config.vscodeTerminalExecutionMode === "backgroundExec");
        ...
        if (config.isSubagentExecution ||
            ...
            void showApprovalNotification({ message: actualCommand, requiresExplicitApproval: autoApproveSafe && requiresApprovalPerLLM }, config.autoApprovalSettings.enableNotifications);

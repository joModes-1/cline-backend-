...
function getInitialTaskPreview(config) {
...
export class AttemptCompletionHandler {
    ...
    async execute(config, block) {
        ...
        if (config.doubleCheckCompletionEnabled && !config.taskState.doubleCheckCompletionPending) {
            ...
            return formatResponse.toolError("Before completing, re-verify your work against the original task requirements. Check that:\n" +
        ...
        if (command) {
            ...
            if (autoApproveSafe) {
                ...
                showNotificationForApproval(`Cline wants to execute a command: ${command}`, config.autoApprovalSettings.enableNotifications);
    ...
    async runNotificationHook(config, notification) {
        const hooksEnabled = getHooksEnabledSafe(config.services.stateManager.getGlobalSettingsKey("hooksEnabled"));
        if (!hooksEnabled) {
            return;
        }

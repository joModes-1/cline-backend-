            ...
            if (hooksEnabled) {
                try {
                    ...
                    if (error instanceof HookCancellationError) {
                        ...
                        await config.callbacks.say("error", "Context compaction was cancelled by PreCompact hook. Task has been aborted.");
                    ...
                    await config.callbacks.say("error", `PreCompact hook failed, continuing with compaction: ${error instanceof Error ? error.message : String(error)}`);

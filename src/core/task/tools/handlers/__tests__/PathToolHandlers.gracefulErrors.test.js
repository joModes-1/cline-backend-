...
function createConfig() {
    ...
    const config = {
        ...
        services: {
            ...
            urlContentFetcher: {},
            diffViewProvider: {},
            clineIgnoreController: { validateAccess: () => true, filterPaths: (paths) => paths },
    ...
    const validator = new ToolValidator({ validateAccess: () => true });
...
describe("ListCodeDefinitionNamesToolHandler.execute – error recovery", () => {
    ...
    it("returns a tool result (not a thrown exception) for a non-existent directory", async () => {
        ...
        assert.ok(result.includes("does not exist or you do not have permission"));
    ...
    it("returns a graceful message for a file path (not a directory)", async () => {
        ...
        assert.equal(typeof result, "string");
        assert.ok(result.includes("file, not a directory"));
        assert.equal(taskState.consecutiveMistakeCount, 1);
    });
    ...
    it("catches a thrown exception from the core operation and returns a tool error", async () => {
        ...
        assert.ok(result.includes("tree-sitter crashed"));
        assert.equal(taskState.consecutiveMistakeCount, 1);
    });
...
describe("ListFilesToolHandler.execute – error recovery", () => {
    ...
    function makeBlock(relPath, recursive) {
        const params = {};
    ...
    it("catches a thrown exception from listFiles and returns a tool error", async () => {
        ...
        assert.equal(typeof result, "string");
        assert.ok(result.includes("Error"));
        assert.ok(result.includes("cwd must be a directory"));
        assert.equal(taskState.consecutiveMistakeCount, 1);
    });
...
describe("SearchFilesToolHandler.execute – error recovery", () => {
    ...
    function makeBlock(relPath, regex, filePattern) {
        const params = {};
        if (relPath !== undefined)
            params.path = relPath;
        if (regex !== undefined)
        ...
        if (filePattern !== undefined)
            params.file_pattern = filePattern;
        return {
            type: "tool_use",
    ...
    it("catches a thrown exception from path resolution and returns a tool error", async () => {
        ...
        sandbox.stub(handler, "determineSearchPaths").throws(new Error("invalid workspace hint"));
        const result = await handler.execute(config, makeBlock("some-dir", "pattern"));
        assert.equal(typeof result, "string");
        assert.ok(result.includes("Error"));
        assert.ok(result.includes("invalid workspace hint"));
        assert.equal(taskState.consecutiveMistakeCount, 1);
    });
    it("accumulates failures when path resolution throws repeatedly", async () => {
        const { config, taskState, validator } = createConfig();
        const handler = new SearchFilesToolHandler(validator);
        sandbox.stub(handler, "determineSearchPaths").throws(new Error("boom"));
...
//# sourceMappingURL=PathToolHandlers.gracefulErrors.test.js.map
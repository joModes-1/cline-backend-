...
export function canonicalizeAttemptCompletionParams(block) {
...
export class ToolExecutor {
    ...
    taskState, messageStateHandler, api, urlContentFetcher, browserSession, diffViewProvider, mcpHub, fileContextTracker, clineIgnoreController, commandPermissionController, contextManager, stateManager, 
    // Configuration & Settings
    cwd, taskId, ulid, vscodeTerminalExecutionMode, 
    // Workspace Management
    workspaceManager, isMultiRootEnabled, 
    // Callbacks to the Task (Entity)
    say, ask, saveCheckpoint, sayAndCreateMissingParamError, removeLastPartialMessageIfExistsWithType, executeCommandTool, cancelRunningCommandTool, doesLatestTaskCompletionHaveNewChanges, updateFCListFromToolResponse, switchToActMode, cancelTask, 
    ...
    setActiveHookExecution, clearActiveHookExecution, getActiveHookExecution, runUserPromptSubmitHook) {
        ...
        this.cancelRunningCommandTool = cancelRunningCommandTool;
    ...
    pushToolResult = (content, block) => {
        ...
        ToolResultUtils.pushToolResult(content, block, this.taskState.userMessageContent, (block) => ToolDisplayUtils.getToolDescription(block), this.coordinator, this.taskState.toolUseIdMap);
     ...
     */
    async runPostToolUseHook(block, toolResult, executionSuccess, executionStartTime, hooksEnabled) {
    ...
    async handleCompleteBlock(block, config) {
        ...
        try {
            ...
            if (hooksEnabled && block.name !== "attempt_completion") {
                const hookRequestedCancel = await this.runPostToolUseHook(block, toolResult, executionSuccess, executionStartTime, hooksEnabled);

...
describe("grpc-recorder", () => {
    ...
    before(async () => {
        recorder = GrpcRecorder.builder()
            .withFilters((req) => req.service === "the-unwanted-service")
            .enableIf(true)
            .build();
    });
    describe("GrpcRecorder", () => {
        it("matches multiple request, response and stats", async () => {
            const requestResponseUseCases = [
                {
                    request: {
                        service: "the-service",
                        method: "the-method",
            ...
            requestResponseUseCases.forEach((us, index) => {
        ...
        it("using default filtering should filter out unwanted requests", async () => {
            const customRecorder = GrpcRecorder.builder()
                .withFilters((req) => req.is_streaming, (req) => ["cline.UiService", "cline.McpService", "cline.WebService"].includes(req.service))
                .enableIf(true)
                .build();
...
//# sourceMappingURL=grpc-recorder.test.js.map
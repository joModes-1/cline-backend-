...
export function buildDiffContent(changedFiles) {
    ...
    for (const file of changedFiles) {
        ...
        parts.push(`--- After (use these line numbers for comments) ---\n${file.after ? addLineNumbers(file.after) : "(deleted)"}\n`);
    }
    return parts.join("\n");
}
/**
...
export async function setupCommentController(apiConfiguration, changedFiles, conversationContext) {
    ...
    commentController.setOnReplyCallback(async (filePath, startLine, endLine, replyText, existingComments, onChunk) => {
        await handleCommentReply(apiConfiguration, filePath, startLine, endLine, replyText, existingComments, changedFiles, conversationContext, onChunk);
    });
 ...
 */
export async function streamAIExplanationComments(apiConfiguration, diffContent, contextDescription, changedFiles, onCommentStart, onCommentChunk, onCommentEnd, shouldAbort) {
 ...
 */
async function handleCommentReply(apiConfiguration, filePath, startLine, endLine, replyText, existingComments, changedFiles, conversationContext, onChunk) {
    // Disable thinking/reasoning for faster response
    const configWithoutThinking = {
        ...apiConfiguration,
        actModeThinkingBudgetTokens: 0,
 ...
 */
export async function detectBinaryFile(filePath) {

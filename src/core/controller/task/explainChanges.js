...
import { buildDiffContent, openDiffView, setupCommentController, streamAIExplanationComments, stringifyConversationHistory, } from "./explainChangesShared";
/**
...
export async function explainChanges(controller, request) {
    ...
    try {
        ...
        if (!checkpointManager.state?.checkpointTracker &&
            ...
            try {
                ...
                checkpointManager.state.checkpointTracker = await CheckpointTracker.create(checkpointManager.task.taskId, checkpointManager.config.enableCheckpoints, workspacePath);
        ...
        const lastTaskCompletedMessageCheckpointHash = findLast(clineMessages.slice(0, messageIndex), (m) => m.say === "completion_result")?.lastCheckpointHash;
        ...
        await streamAIExplanationComments(apiConfiguration, diffContent, conversationSummary, changedFiles, 

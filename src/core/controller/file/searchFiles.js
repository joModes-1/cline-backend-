...
export async function searchFiles(controller, request) {
    try {
        ...
        if (hasMultirootSupport) {
            searchResults = await searchWorkspaceFilesMultiroot(request.query || "", workspaceManager, request.limit || 20, selectedTypeString, workspaceHint);
        }
        else {
            ...
            searchResults = await searchWorkspaceFiles(request.query || "", workspacePath, request.limit || 20, // Use default limit of 20 if not specified
            selectedTypeString);
        }
        ...
        await telemetryService.captureMentionSearchResults(request.query || "", protoResults.length, searchType, protoResults.length === 0);
        ...
        const mentionType = request.selectedType === FileSearchType.FILE
            ? "file"

...
import { ensureCacheDirectoryExists, ensureMcpServersDirectoryExists, ensureSettingsDirectoryExists, GlobalFileNames, writeMcpMarketplaceCatalogToCache, } from "../storage/disk";
...
export class Controller {
    ...
    constructor(context) {
        ...
        this.mcpHub = new McpHub(() => ensureMcpServersDirectoryExists(), () => ensureSettingsDirectoryExists(), ExtensionRegistryInfo.version, telemetryService);
    ...
    */
    ...
    async initTask(task, images, files, historyItem, taskSettings, cwdOverride) {
        ...
        // If cwdOverride is provided, use that as the workspace root instead of detecting
        if (cwdOverride) {
            HostProvider.overrideWorkspacePaths([cwdOverride]);
        }
        this.workspaceManager = await setupWorkspaceManager({
            stateManager: this.stateManager,
            detectRoots: cwdOverride ? async () => [{ path: cwdOverride, vcs: VcsType.None, isPrimary: true }] : detectWorkspaceRoots,
        });
        const cwd = cwdOverride || this.workspaceManager?.getPrimaryRoot()?.path || (await getCwd(getDesktopDir()));
    ...
    // Hicap
    async handleHicapCallback(code) {
        const apiKey = code;
        const hicap = "hicap";
        const currentMode = this.stateManager.getGlobalSettingsKey("mode");
        // Update API configuration through cache service

...
function normalizeRecommendedModelsResponse(raw) {
    if (!raw || typeof raw !== "object") {
        return null;
    }
    const data = raw;
    if ((data.recommended !== undefined && !Array.isArray(data.recommended)) ||
...
export async function refreshClineRecommendedModels() {
...
export function resetClineRecommendedModelsCacheForTests() {
    pendingRefresh = null;
    inMemoryCache = null;
}
async function fetchAndCacheClineRecommendedModels() {
    const clineRecommendedModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.clineRecommendedModels);
    let result = { recommended: [], free: [] };
    try {
...
//# sourceMappingURL=refreshClineRecommendedModels.js.map
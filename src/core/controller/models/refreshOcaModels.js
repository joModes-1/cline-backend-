...
import { CHAT_COMPLETIONS_API, DEFAULT_EXTERNAL_OCA_BASE_URL, DEFAULT_INTERNAL_OCA_BASE_URL, MESSAGES_API, RESPONSES_API, } from "@/services/auth/oca/utils/constants";
...
export async function refreshOcaModels(controller, request) {
    ...
    try {
        ...
        if (response.data?.data) {
            ...
            const planModeSelectedModelId = apiConfiguration?.planModeOcaModelId && models[apiConfiguration.planModeOcaModelId]
                ? apiConfiguration.planModeOcaModelId
                : defaultModelId;
            const actModeSelectedModelId = apiConfiguration?.actModeOcaModelId && models[apiConfiguration.actModeOcaModelId]
                ? apiConfiguration.actModeOcaModelId
                : defaultModelId;
            let planModeOcaReasoningEffort;
            let actModeOcaReasoningEffort;
            if (models[planModeSelectedModelId].supportsReasoning &&
            ...
            if (models[actModeSelectedModelId].supportsReasoning &&
...
function supportsChatCompletions(modelSupportedApiList) {
    return modelSupportedApiList.includes(CHAT_COMPLETIONS_API);
}
function supportsResponses(modelSupportedApiList) {
    return modelSupportedApiList.includes(RESPONSES_API);
}
function supportsMessages(modelSupportedApiList) {

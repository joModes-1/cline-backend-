...
import { ANTHROPIC_MAX_THINKING_BUDGET, CLAUDE_OPUS_1M_TIERS, CLAUDE_SONNET_1M_TIERS, openRouterClaudeOpus461mModelId, openRouterClaudeSonnet41mModelId, openRouterClaudeSonnet451mModelId, openRouterClaudeSonnet461mModelId, } from "@/shared/api";
...
async function fetchRawClineModels() {
 ...
 */
export async function refreshClineModels(controller) {
...
async function fetchAndCacheClineModels() {
    ...
    try {
        ...
        for (const rawModel of rawModels) {
            ...
            if (isGeminiFlashModel(rawModel.id)) {
                modelInfo.maxTokens = Math.min(modelInfo.maxTokens || GEMINI_FLASH_MAX_OUTPUT_TOKENS, GEMINI_FLASH_MAX_OUTPUT_TOKENS);
            }
 ...
 */
export async function readClineModelsFromCache() {
    try {
        const clineModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.clineModels);
        const fileExists = await fileExistsAtPath(clineModelsFilePath);
        if (fileExists) {
...
//# sourceMappingURL=refreshClineModels.js.map
...
export async function updateSettingsCli(controller, request) {
    ...
    try {
        ...
        if (request.settings) {
            ...
            autoApprovalSettings, planModeReasoningEffort, actModeReasoningEffort, mode, customPrompt, planModeApiProvider, actModeApiProvider, 
            // Fields requiring special logic (telemetry, merging, etc.)
            telemetrySetting, yoloModeToggled, useAutoCondense, clineWebToolsEnabled, worktreesEnabled, subagentsEnabled, focusChainSettings, browserSettings, defaultTerminalProfile, ...simpleSettings } = request.settings;
            // Batch update for simple pass-through fields
            const filteredSettings = Object.fromEntries(Object.entries(simpleSettings).filter(([key, value]) => key !== "openaiReasoningEffort" && value !== undefined));
            ...
            if (useAutoCondense !== undefined) {
                if (controller.task) {
                    telemetryService.captureAutoCondenseToggle(controller.task.ulid, useAutoCondense, controller.task.api.getModel().id);
                }

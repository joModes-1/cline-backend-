...
import { PlanActMode, McpDisplayMode as ProtoMcpDisplayMode } from "@shared/proto/cline/state";
...
export async function updateSettings(controller, request) {
    try {
        ...
        if (request.apiConfiguration) {
            ...
            const convertedApiConfigurationFromProto = {
                ...
                actModeApiProvider: protoApiConfiguration.actModeApiProvider
                    ...
                    : undefined,
                planModeReasoningEffort: protoApiConfiguration.planModeReasoningEffort,
                actModeReasoningEffort: protoApiConfiguration.actModeReasoningEffort,
            };
        ...
        if (request.vscodeTerminalExecutionMode !== undefined && request.vscodeTerminalExecutionMode !== "") {
            controller.stateManager.setGlobalState("vscodeTerminalExecutionMode", request.vscodeTerminalExecutionMode === "backgroundExec" ? "backgroundExec" : "vscodeTerminal");
        }
        ...
        if (request.useAutoCondense !== undefined) {
            if (controller.task) {
                telemetryService.captureAutoCondenseToggle(controller.task.ulid, request.useAutoCondense, controller.task.api.getModel().id);
            }
        ...
        if (request.browserSettings !== undefined) {
            ...
            const newBrowserSettings = {
                ...
                remoteBrowserEnabled: request.browserSettings.remoteBrowserEnabled === undefined
                    ? currentSettings.remoteBrowserEnabled
                    : request.browserSettings.remoteBrowserEnabled,
                remoteBrowserHost: request.browserSettings.remoteBrowserHost === undefined
                ...
                customArgs: "customArgs" in request.browserSettings ? request.browserSettings.customArgs : currentSettings.customArgs,
            };
        ...
        if (request.defaultTerminalProfile !== undefined) {
            ...
            if (controller.task) {
                ...
                if (busyTerminalsCount > 0) {
                    const message = `${busyTerminalsCount} busy ${busyTerminalsCount === 1 ? "terminal has" : "terminals have"} a different profile. ` +
        ...
        if (request.nativeToolCallEnabled !== undefined) {
            ...
            if (controller.task) {
                telemetryService.captureFeatureToggle(controller.task.ulid, "native-tool-call", request.nativeToolCallEnabled, controller.task.api.getModel().id);
            }
        }

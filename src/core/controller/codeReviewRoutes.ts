...
import { RepositoryManager, type GitHubRepoInfo, type Repository } from "../../services/codeReview/RepositoryManager"
...
// Define types for express-fileupload locally to avoid import issues
...
type AutofixSnapshotKey = string
type AutofixSnapshot = { filePath: string; before: string }
const autofixSnapshots: Map<AutofixSnapshotKey, AutofixSnapshot> = new Map()
		...
		if (fs.existsSync(SNAPSHOTS_FILE)) {
			...
			if (data && typeof data === 'object') {
				for (const [key, value] of Object.entries(data)) {
					autofixSnapshots.set(key, value as AutofixSnapshot)
				}
...
function saveSnapshotsToDisk(): void {
	try {
		const data: Record<string, AutofixSnapshot> = {}
...
// Extend Express Request to include controller and files
interface CodeReviewRequest extends Request {
...
function getAutofixSnapshotKey(repoId: string, taskId: string, filePath: string): AutofixSnapshotKey {
	return `${repoId}::${taskId}::${filePath}`
}

export function createCodeReviewRouter(controller: Controller): Router {
	const router = Router()
	 ...
	 */
	router.get("/github/auth", authenticate, (req: AuthRequest, res: Response): void => {
	 ...
	 */
	router.get("/github/status", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
	 ...
	 */
	router.get("/github/repos", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
	 ...
	 */
	router.post("/repos/github/clone", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		Logger.info("[API] POST /api/repos/github/clone - GitHub clone request received")

		try {
			const { owner, repo, branch = "main", token } = req.body as GitHubRepoInfo
			const userId = req.user!.id
			...
			if (user) {
				...
				if (!existingRepo) {
					user.repositories.push(repoData as any)
	 ...
	 */
	router.get("/repos/:repoId/autofix/:taskId/diff", authenticate, async (req: AuthRequest, res: Response) => {
		try {
			...
			if (!snapshot) {
				Logger.warn(
					`[API] No snapshot found for diff (repoId=${repoId}, taskId=${taskId}, file=${filePath}). Falling back to empty before-content.`,
				)
			}
			...
			const repoMeta = user.repositories.find((r: any) => r.id === repoId)
			if (!repoMeta) {
				res.status(404).json({ success: false, error: "Repository not found" })
				return
			}

			const localPath = (repoMeta as any).localPath
			if (!localPath) {
	 ...
	 */
	router.get(
		"/repos/:repoId/autofix/:taskId/status",
		authenticate,
		async (req: AuthRequest, res: Response): Promise<void> => {
			try {
				...
				const activeTask = controller.task
				const isThisTask = activeTask?.taskId === taskId
				// Include last few Cline messages so callers can diagnose what the AI did
				const clineMessages = isThisTask ? (activeTask as any).clineMessages ?? [] : []
				const lastMessages = clineMessages.slice(-10).map((m: any) => ({
					role: m.type,
					text: typeof m.text === "string" ? m.text.slice(0, 300) : JSON.stringify(m.text ?? "").slice(0, 300),
					ts: m.ts,
				}))
	 ...
	 */
	router.get(
		"/repos/:repoId/autofix/:taskId/snapshot",
		authenticate,
		async (req: AuthRequest, res: Response): Promise<void> => {
			try {
				const repoId = String(req.params.repoId)
	 ...
	 */
	router.post(
		"/repos/:repoId/autofix/:taskId/accept",
		authenticate,
		async (req: AuthRequest, res: Response): Promise<void> => {
			try {
				const repoId = String(req.params.repoId)
	 ...
	 */
	router.post(
		"/repos/:repoId/autofix/:taskId/reject",
		authenticate,
		async (req: AuthRequest, res: Response): Promise<void> => {
			try {
				const repoId = String(req.params.repoId)
	 ...
	 */
	router.post("/repos/:repoId/autofix", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
			...
			const suggestedReplacement: unknown = issue?.suggestedFix?.replacement
			if (
			...
			const prompt =
				`You are Cline running inside Villa Code Reviewer. Apply an auto-fix for the single issue below.\n\n` +
				`Rules:\n` +
	 ...
	 */
	router.post("/repos/zip/upload", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		Logger.info("[API] POST /api/repos/zip/upload - Zip upload request received")

		try {
			const candidate: unknown =
				req.files && (req.files as any).zip
					? (req.files as any).zip
					: req.files && (req.files as any).file
			...
			const uploaded = zipFile as UploadedFile
			const userId = req.user!.id
	...
	router.get("/repos", authenticate, async (req: AuthRequest, res: Response) => {
		...
		try {
			...
			const reposWithStats = await Promise.all(
				user.repositories.map(async (repo: any) => {
					let fileCount = 0
					try {
						if (repo.localPath) {
	 ...
	 */
	router.get("/repos/:repoId", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
			...
			if (localPath) {
				try {
					...
					Logger.warn("Failed to get stats/files:", e as Error)
				}
			}
	 ...
	 */
	router.delete("/repos/:repoId", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
			...
			const repoIndex = user.repositories.findIndex((r: any) => r.id === repoId)
			if (repoIndex === -1) {
				res.status(404).json({
					success: false,
					error: "Repository not found",
			...
			const repoMeta = user.repositories[repoIndex] as any
	 ...
	 */
	router.post("/repos/:repoId/scan", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
			...
			Logger.info(`[API] Using API provider: ${(apiConfig as any).actModeApiProvider || 'default'}`)
			...
			const flatIssues = scanResult.files.flatMap(file => 
	 ...
	 */
	router.post("/repos/:repoId/review", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
	 ...
	 */
	router.get("/repos/:repoId/files", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
	 ...
	 */
	router.get("/repos/:repoId/file", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
	 ...
	 */
	router.put("/repos/:repoId/file", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
	 ...
	 */
	router.post("/repos/:repoId/fix", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
	 ...
	 */
	router.post("/review/direct", async (req: Request, res: Response): Promise<void> => {
		try {
	 ...
	 */
	router.get("/user/apikeys", (req: Request, res: Response): void => {
	 ...
	 * Save API key
	 */
	router.post("/user/apikeys", (req: Request, res: Response): void => {
		const sessionId = req.headers.authorization?.slice(7) || 'anonymous'
	 ...
	 * Remove API key
	 */
	router.delete("/user/apikeys/:provider", (req: Request, res: Response): void => {
		const sessionId = req.headers.authorization?.slice(7) || 'anonymous'
		const { provider } = req.params
		const providerKey = Array.isArray(provider) ? provider[0] : provider

		const currentKeys = userApiKeys.get(sessionId) || {}
		delete (currentKeys as Record<string, string>)[providerKey]
		userApiKeys.set(sessionId, currentKeys)
	 ...
	 */
	router.get("/code-review/settings", (req: Request, res: Response): void => {
		try {
			const apiConfig = controller.stateManager.getApiConfiguration()
			...
			res.json({
				success: true,
				data: {
					apiProvider: (apiConfig as any).actModeApiProvider,
					apiModel: (apiConfig as any).actModeApiModelId,
	 ...
	 */
	router.post("/code-review/settings", async (req: Request, res: Response): Promise<void> => {
		try {
	 ...
	 */
	router.post("/code-review/configure-api", async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (apiModel) {
				...
				if (providerKey) {
					await controller.stateManager.setGlobalState(providerKey as any, apiModel)
	 ...
	 */
	router.get("/code-review/validate-api", async (req: Request, res: Response): Promise<void> => {
		try {
			const apiConfig = controller.stateManager.getApiConfiguration()
			...
			try {
				...
				res.json({
					...
					data: {
						valid: true,
						provider: (apiConfig as any).actModeApiProvider,
						model: (apiConfig as any).actModeApiModelId,
		...
		try {
			...
			res.json({
				...
				data: {
					...
					cache: (codeReviewService as any).getCacheStats?.() || { hits: 0, misses: 0 },
				},
			})
		} catch (error) {
			res.status(500).json({
	...
	router.post("/chat", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			...
			const clineMessages = messages.map((m: any) => ({
	...
	// ─── Villa Validate — unified static audit + AI scan (Layer F) ───────────────
	// Replaces the two-button "Scan + Audit" flow with a single endpoint that:
	//   1. Runs the static AuditEngine (parsers + rules — fast, deterministic).
	//   2. Runs the AI compliance reviewer in parallel (slow but contextual).
	//   3. Drops audit findings without a `file` (citation enforcement).
	//   4. Maps audit findings into the same issue shape the autofix pipeline
	//      and the UI already understand — so audit findings become fixable
	//      through POST /repos/:repoId/autofix without a new fix engine.
	//   5. Tags every issue with a `source` so callers can tell the engines apart.
	router.post('/repos/:repoId/validate', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const repoId = String(req.params.repoId)
			const userId = req.user!.id
			const { maxFiles = 50, maxFileSize = 250000, skipParsers } = req.body as {
				maxFiles?: number
				maxFileSize?: number
				skipParsers?: string[]
			}
			...
			Logger.info(`[API] ════════════════════════════════════════════════════════`)
			Logger.info(`[API] STARTING VALIDATE: ${repoId}`)
			Logger.info(`[API] ════════════════════════════════════════════════════════`)

			const { runAudit } = await import('../../services/audit/AuditEngine')

			// Run both engines in parallel. Either failing must not bring the
			// other down — partial results are more useful than nothing.
			const [auditOutcome, scanOutcome] = await Promise.allSettled([
				runAudit({ repoId, repoPath: localPath, skipParsers }),
				codeReviewService.scanRepository({
					repoPath: localPath,
					maxFiles,
					maxFileSize,
				}),
			...
			const auditReport = auditOutcome.status === 'fulfilled' ? auditOutcome.value : null
			const scanResult = scanOutcome.status === 'fulfilled' ? scanOutcome.value : null

			if (auditOutcome.status === 'rejected') {
				Logger.error('[API] Validate: static audit branch failed:', auditOutcome.reason)
			}
			if (scanOutcome.status === 'rejected') {
				Logger.error('[API] Validate: AI scan branch failed:', scanOutcome.reason)
			}

			// ── Severity normalization map (AuditFinding → CodeReviewIssue) ──
			const sevMap: Record<string, 'error' | 'warning' | 'info'> = {
				BLOCKER: 'error',
				WARNING: 'warning',
				INFO: 'info',
			}

			// ── 1. Static audit findings → issue shape ──
			// Citation enforcement: drop any finding without a `file`. A finding
			// the user can't locate isn't actionable, and the autofix pipeline
			// requires a file path anyway.
			let droppedNoFile = 0
			const auditIssues = (auditReport?.findings ?? [])
				.filter((f) => {
					if (typeof f.file === 'string' && f.file.length > 0) return true
					droppedNoFile++
					return false
				})
				.map((f, idx) => ({
					id: `audit_${idx}_${Date.now()}`,
					source: 'audit-static' as const,
					...
					ruleId: f.id,
					category: f.category,
					storeRule: f.storeRule,
					message: f.title,
					description: f.description,
					code: f.suggestedFix?.replacement,
					suggestion: f.suggestedFix?.description ?? f.fixSuggestion,
			...
			// ── 2. AI scan findings → issue shape (same flat shape as /scan) ──
			const scanIssues = (scanResult?.files ?? []).flatMap((file) =>
				file.issues.map((issue) => ({
					...issue,
					source: 'scan-ai' as const,
					file: file.path,
					line: issue.line || 1,
					column: issue.column || 1,
					severity: issue.severity || 'warning',
			...
			// ── 3. Deduplicate: same file + line + ruleId reported by both
			//    engines collapses to one issue, preferring the audit-static
			//    entry because its citation is grounded in deterministic parsing.
			const seen = new Set<string>()
			const merged = [...auditIssues, ...scanIssues].filter((i) => {
				const key = `${(i.file || '').toLowerCase()}::${i.line}::${i.ruleId}`
				if (seen.has(key)) return false
				seen.add(key)
				return true
			})

			// ── 4. Sort: errors first, then warnings, then info ──
			const sevOrder = { error: 0, warning: 1, info: 2, hint: 3 } as Record<string, number>
			merged.sort((a, b) => (sevOrder[a.severity] ?? 9) - (sevOrder[b.severity] ?? 9))

			const errors = merged.filter((i) => i.severity === 'error').length
			const warnings = merged.filter((i) => i.severity === 'warning').length
			const infos = merged.length - errors - warnings
			...
			const result = {
				...
				summary: {
					...
					filesWithIssues: new Set(merged.map((i) => i.file)).size,
					criticalCount: errors,
					warningCount: warnings,
					infoCount: infos,
					auditFindings: auditIssues.length,
					scanFindings: scanIssues.length,
					droppedNoFile,
					storeReady: errors === 0,
				},
				scores: auditReport?.scores ?? { overall: 0 },
				platform: auditReport?.platform ?? null,
				completedAt: new Date().toISOString(),
				partial: {
					auditFailed: auditOutcome.status === 'rejected',
					scanFailed: scanOutcome.status === 'rejected',
				},
			}

			Logger.info(
				`[API] VALIDATE done: ${merged.length} merged (audit ${auditIssues.length}, scan ${scanIssues.length}, dropped ${droppedNoFile})`,
			)
	...
	// ─── Villa Audit Engine ──────────────────────────────────────────────────────
	router.post('/audit/:repoId', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
		try {
			const { repoId } = req.params
			const { skipParsers } = req.body as { skipParsers?: string[] }
			const userId = req.user!.id

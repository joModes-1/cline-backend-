...
// Snapshot Helpers
// ============================================================================
const formatSnapshotError = (snapshotName, details) => `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async function assertSnapshot(name, content) {
...
const makeProviderInfo = (modelId, providerId = "test") => ({
...
const isNativeToolsFamily = (family) => [ModelFamily.NATIVE_NEXT_GEN, ModelFamily.NATIVE_GPT_5, ModelFamily.NATIVE_GPT_5_1, ModelFamily.GEMINI_3].includes(family);
async function runPromptTest(testCtx, context, modelId, handler) {
...
const contextVariations = [
    ...
    { name: "no-mcp", override: { mcpHub: { getServers: () => [] } } },
            ...
            for (const { family, modelId, providerId } of gemini3ModelTestCases) {
                ...
                it(`should include parallel tool-calling guidance for ${providerId}/${modelId} when enabled`, async function () {
                    ...
                    await runPromptTest(this, context, modelId, async ({ systemPrompt }) => {
                        expect(systemPrompt).to.include("- When multiple operations are independent (for example reading several files or searching in multiple directories), call multiple tools in a single response rather than one at a time.");

...
let resolvedHookLauncherCommandPromise = null;
let resolvedHookLauncherCommandExpiresAt = 0;
export function resetHookLaunchConfigCacheForTesting() {
    resolvedHookLauncherCommandPromise = null;
    resolvedHookLauncherCommandExpiresAt = 0;
}
function shouldRefreshWindowsLauncherCache(now) {
 ...
 */
export async function getHookLaunchConfig(scriptPath, resolvePowerShellExecutable = resolveWindowsPowerShellExecutable) {
...
export class HookProcess extends EventEmitter {
    ...
    isRegistered = false;
    constructor(scriptPath, timeoutMs = 30000, abortSignal, cwd) {
        super();

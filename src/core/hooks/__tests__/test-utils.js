...
export function resetHookCache() {
...
export function hookFileName(hookName, platform = process.platform) {
    return platform === "win32" ? `${hookName}.ps1` : hookName;
}
export function hookPath(hooksDir, hookName, platform = process.platform) {
    return path.join(hooksDir, hookFileName(hookName, platform));
}
export function stubHookDirs(sandbox, dirs) {
    const existing = diskModule.getAllHooksDirs;
...
export async function createHookTestEnv() {
 ...
 */
export async function writeHookScriptForPlatform(hookPath, nodeScript) {
...
function buildPowerShellNodeBridge(nodePath, jsFileName) {

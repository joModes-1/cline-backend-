...
export async function migrateTaskHistoryToFile(context) {
    try {
        ...
        if (successfullyWrittenData.length !== finalData.length) {
            Logger.error("[Storage Migration] Failed to write taskHistory to file: Written data does not match the old location data");
            return;
        }
...
export async function migrateMcpMarketplaceEnableSetting(mcpMarketplaceEnabledRaw) {
    // Standalone/web builds do not have access to VS Code workspace configuration.
...
export async function cleanupOldApiKey(context) {
    try {

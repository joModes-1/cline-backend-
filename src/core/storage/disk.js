 ...
 * file first, then renaming it to the target location.
 *
 * On Windows, fs.rename can fail with EPERM/EBUSY when antivirus, the search indexer,
 * or another process briefly holds an open handle on the target. We retry the rename
 * with exponential backoff, and as a last resort fall back to copy+unlink which can
 * succeed in cases rename does not. Task progress is more important than strict
 * atomicity for this storage path.
 */
async function atomicWriteFile(filePath, data) {
    ...
    try {
        ...
        let lastError;
        // 6 attempts: 20, 40, 80, 160, 320, 640ms — total < 1.3s.
        ...
        // Fallback: copy temp -> target, then unlink temp. Loses atomicity but
        // keeps the task alive when a Windows file lock is sticky.
        try {
            await fs.copyFile(tmpPath, filePath);
            await fs.unlink(tmpPath).catch(() => { });
            Logger.warn(`[atomicWriteFile] Rename failed after retries, used copyFile fallback for ${filePath} (code=${lastError?.code})`);
            return;
        }
        catch (copyErr) {
 ...
 */
export function getClineHomePath() {
 ...
 */
function getClineSkillsDirectoryPath() {
 ...
 */
export function setRuntimeHooksDir(dir) {
    runtimeHooksDir = dir;
}
/**

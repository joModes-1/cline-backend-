...
import { ensureStateDirectoryExists, getAllHooksDirs, getTaskHistoryStateFilePath, getWorkspaceHooksDirs, readTaskHistoryFromState, setRuntimeHooksDir, writeTaskHistoryToState, } from "../disk";
...
describe("disk - hooks functionality", () => {
    ...
    describe("getAllHooksDirs", () => {
        it("should include the runtime hooks directory when it exists", async () => {
            ...
            sandbox.stub(fsUtils, "isDirectory").callsFake(async (targetPath) => targetPath === runtimeHooksDir);
...
describe("disk - atomic writes", () => {
     ...
     */
    const createTestHistoryItem = (id, task) => {
        return {
            id,
            ts: Date.now(),
            task,
    ...
    describe("writeTaskHistoryToState and readTaskHistoryFromState", () => {
        ...
        it("should handle large task history arrays", async function () {
            ...
            const items = Array.from({ length: 1000 }, (_, i) => createTestHistoryItem(`stress-test-${i}`, `Task ${i}: ${baseContent}`));
            await writeTaskHistoryToState(items);
            const result = await readTaskHistoryFromState();
...
//# sourceMappingURL=disk.test.js.map
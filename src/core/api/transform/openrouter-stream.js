import { CLAUDE_SONNET_1M_SUFFIX, OPENROUTER_PROVIDER_PREFERENCES, openRouterClaudeOpus461mModelId, openRouterClaudeSonnet41mModelId, openRouterClaudeSonnet451mModelId, openRouterClaudeSonnet461mModelId, } from "@shared/api";
import { normalizeOpenaiReasoningEffort } from "@shared/storage/types";
import { GEMINI_FLASH_MAX_OUTPUT_TOKENS, isGeminiFlashModel, shouldSkipReasoningForModel, supportsReasoningEffortForModel, } from "@utils/model-utils";
...
export async function createOpenRouterStream(client, systemPrompt, messages, model, reasoningEffort, thinkingBudgetTokens, openRouterProviderSorting, tools, enableParallelToolCalling) {
    // Convert Anthropic messages to OpenAI format
    let openAiMessages = [
        { role: "system", content: systemPrompt },
        ...convertToOpenAiMessages(messages),
    ];
    const isClaude1m = model.id === openRouterClaudeSonnet41mModelId ||
    ...
    const reasoningPayload = reasoning ?? (reasoningEffortValue && reasoningEffortValue !== "none" ? { effort: reasoningEffortValue } : undefined);

...
export class VsCodeLmHandler implements ApiHandler, SingleCompletionHandler {
	...
	private vscodeModule: (typeof import("vscode")) | null
	...
	constructor(options: VsCodeLmHandlerOptions) {
		...
		this.vscodeModule = null

		// Note: we intentionally do not reference the runtime 'vscode' module here.
		// This backend can run outside VS Code, where the 'vscode' package is not available.
	}

	private async getVscodeModule(): Promise<typeof import("vscode")> {
		if (this.vscodeModule) {
			return this.vscodeModule
		}
		try {
			this.vscodeModule = await import("vscode")
		} catch (err) {
			throw new Error(
				"Cline <Language Model API>: VS Code language model provider requires the VS Code extension host. 'vscode' module not available.",
			)
		}

		try {
			// Listen for model changes and reset client (only once vscode is available)
			this.disposable = this.vscodeModule.workspace.onDidChangeConfiguration((event) => {
				if (event.affectsConfiguration("lm")) {
					this.client = null
				}
			})
		} catch (err) {
			Logger.error("Cline <Language Model API>: Error setting up configuration change listener", err)
		}
	...
	async createClient(selector: vscode.LanguageModelChatSelector): Promise<vscode.LanguageModelChat> {
		const vscode = await this.getVscodeModule()
		try {
	...
	private extractTextFromMessage(message: vscode.LanguageModelChatMessage): string {
		const vscode = this.vscodeModule
		if (!vscode) {
			return ""
		}
	...
	async *createMessage(systemPrompt: string, messages: ClineStorageMessage[]): ApiStream {
		...
		const vsCodeLmMessages: vscode.LanguageModelChatMessage[] = [
			...
			...convertToVsCodeLmMessages(cleanedMessages, vscode),
		]

...
export class AwsBedrockHandler {
     ...
     */
    async *createDeepseekMessage(systemPrompt, messages, modelId, model, _tools) {
     ...
     */
    mapClineToolsToBedrockToolConfig(tools) {
        if (!tools || tools.length === 0) {
            return undefined;
        }
        const isAnthropicTool = (tool) => "input_schema" in tool;
            ...
            if (response.stream) {
                ...
                for await (const chunk of response.stream) {
                    ...
                    if (chunk.contentBlockStart) {
                        ...
                        if (blockStart.start?.type === "redacted_thinking" ||
    ...
    *handleBedrockStreamError(chunk) {
     ...
     */
    async *createAnthropicMessage(systemPrompt, messages, modelId, model, enable1mContextWindow, tools) {
     ...
     */
    async *createNovaMessage(systemPrompt, messages, modelId, model, tools) {
     ...
     */
    async *createOpenAIMessage(systemPrompt, messages, modelId, model, tools) {
        // Get Bedrock client with proper credentials
        const client = await this.getBedrockClient();
     ...
     * Uses non-streaming Converse API and simulates streaming for models that don't support it
     */
    async *createQwenMessage(systemPrompt, messages, modelId, model, tools) {
        // Get Bedrock client with proper credentials
        const client = await this.getBedrockClient();
        // Format messages for Converse API
        const formattedMessages = this.formatMessagesForConverseAPI(messages);

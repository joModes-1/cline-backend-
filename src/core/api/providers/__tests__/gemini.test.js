...
describe("GeminiHandler", () => {
    ...
    it("caps maxOutputTokens to 8192 for Flash models", async () => {
        ...
        sinon.stub(handler, "ensureClient").returns({
        ...
        const requestArgs = generateContentStream.firstCall.args[0];
    ...
    it("should emit unique tool call IDs when multiple function calls share one responseId", async () => {
        ...
        const fakeClient = {
            models: {
                generateContentStream: sinon.stub().resolves(createAsyncIterable([
                    {
                        responseId: "resp_1",
                        candidates: [
                            {
        ...
        const tools = [{ name: "read_file", description: "read file", parameters: { type: "OBJECT" } }];
        const chunks = [];

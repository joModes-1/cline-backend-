...
// NOTE: Read the first element without mutating. The array returned by getWorkspacePaths is the same
// reference as HostProvider.overriddenWorkspacePaths, so .shift() permanently empties the global
// override and breaks every subsequent isLocatedInWorkspace() check for the rest of the process.
export async function getCwd(defaultCwd = ""): Promise<string> {
	const workspacePaths = await HostProvider.workspace.getWorkspacePaths({})
	return workspacePaths.paths[0] || defaultCwd
}

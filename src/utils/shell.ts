...
let vscodeModule: VscodeModule | null | undefined

async function getVscodeModule(): Promise<VscodeModule | null> {
...
function getWindowsTerminalConfig(): { defaultProfileName: string | null; profiles: WindowsTerminalProfiles } {
	try {
		// In standalone/web builds, vscode may not be available.
		return { defaultProfileName: null as string | null, profiles: {} as WindowsTerminalProfiles }
	} catch {
		return { defaultProfileName: null as string | null, profiles: {} as WindowsTerminalProfiles }
	}
}

function getMacTerminalConfig(): { defaultProfileName: string | null; profiles: MacTerminalProfiles } {
	try {
		return { defaultProfileName: null as string | null, profiles: {} as MacTerminalProfiles }
	} catch {
		return { defaultProfileName: null as string | null, profiles: {} as MacTerminalProfiles }
	}
}

function getLinuxTerminalConfig(): { defaultProfileName: string | null; profiles: LinuxTerminalProfiles } {
	try {
		return { defaultProfileName: null as string | null, profiles: {} as LinuxTerminalProfiles }
	} catch {
		return { defaultProfileName: null as string | null, profiles: {} as LinuxTerminalProfiles }
	}
}
...
function getWindowsShellFromVSCode(): string | null {
	// Standalone/web builds do not have access to VS Code configuration.
	return null
}

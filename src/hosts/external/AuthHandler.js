...
export class AuthHandler {
    ...
    async getCallbackUrl(path = "", preferredPort) {
        if (!this.enabled) {
    ...
    async createServer(preferredPort) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.port = 0;
                this.serverCreationPromise = null;
                reject(new Error(`No available port found for local auth callback (tried ${PORT_RANGE_START}-${PORT_RANGE_END}).`));
            }
            catch (error) {

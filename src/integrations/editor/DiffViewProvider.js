...
export class DiffViewProvider {
    ...
    async open(relPath, options) {
        ...
        this.preDiagnostics = (await HostProvider.workspace.getDiagnostics({})).diagnostics;
     ...
     */
    async safelyTruncateDocument(lineNumber) {
    ...
    async getNewDiagnosticProblems() {
        ...
        const postDiagnostics = (await HostProvider.workspace.getDiagnostics({})).diagnostics;
    ...
    lastUpdateContentLength = -1;
    lastUpdateTime = 0;
    static UPDATE_THROTTLE_MS = 100; // Throttle updates to max 10/second during streaming
    ...
    async saveChanges() {
        ...
        if (saveResult === false) {
            throw new Error(`Failed to save changes to ${this.absolutePath}: saveDocument() returned false`);
        }

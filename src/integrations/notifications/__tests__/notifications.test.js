...
function decodePowerShellEncodedCommand(encoded) {
...
describe("notifications", () => {
    ...
    it("escapes quotes and backslashes for macOS notifications", async () => {
        ...
        notificationsModule.setNotificationExecaForTesting(execaStub);
        const platformStub = sinon.stub().returns("darwin");
        notificationsModule.setNotificationPlatformForTesting(platformStub);
        ...
        const script = execaStub.firstCall.args[1][1];
    ...
    it("creates explicit approval marker only when required", () => {
        expect(notificationsModule.createApprovalNotificationMessage({ message: "npm install", requiresExplicitApproval: true })).to.equal("npm install (explicit approval required)");
        expect(notificationsModule.createApprovalNotificationMessage({ message: "npm install", requiresExplicitApproval: false })).to.equal("npm install");
    });
    it("normalizes whitespace in approval notification messages", () => {
        expect(notificationsModule.createApprovalNotificationMessage({
            message: "npm\n\tinstall    ./pkg",
    ...
    it("encodes Windows PowerShell notifications before dispatch", async () => {
        ...
        const args = execaStub.firstCall.args[1];

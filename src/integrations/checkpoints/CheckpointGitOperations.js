    ...
    async renameNestedGitRepos(disable) {
        ...
        for (const gitPath of gitPaths) {
            ...
            try {
                ...
                throw new Error(`Failed to ${disable ? "disable" : "enable"} nested git repo ${gitPath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    /**
        ...
        try {
            ...
            await retryWithBackoff(() => this.renameNestedGitRepos(false), {
                ...
                onRetry: (_error, attempt, maxAttempts, delayMs) => {
                    Logger.warn(`CheckpointTracker re-enable nested git repos failed on attempt ${attempt}/${maxAttempts}. Retrying in ${delayMs}ms`);
                },
            }).catch((error) => {

...
import type {
    ExecutionRequest,
    ExecutionResult,
    ExecutionStatus, LanguageConfig,
    ExecutionLimits
} from './types';
...
export class SandboxRunner extends EventEmitter {
  ...
  constructor(limits: Partial<ExecutionLimits> = {}, baseTempDir?: string) {
    super();
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        childProcess.on('close', async (code: number | null) => {
          ...
          const status: ExecutionStatus = sandbox.status === 'timeout' 
            ? 'timeout' 
   ...
   */
  killSandbox(executionId: string, reason: 'timeout' | 'cancelled' = 'cancelled'): boolean {
   ...
   * Get active sandbox
   */
  getSandbox(executionId: string): ActiveSandbox | undefined {
   ...
   */
  getWorkspaceSandboxes(workspaceId: string): ActiveSandbox[] {
   ...
   */
  private async createSandboxDir(executionId: string): Promise<string> {

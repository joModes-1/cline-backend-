...
import type { Platform, PlatformProfile } from './AuditTypes';
...
interface Fingerprint {
  ...
  dirs?: string[];    // any of these dirs must exist
}
...
function dirExists(repoPath: string, rel: string): boolean {
  try {
...
function globExists(repoPath: string, pattern: string): boolean {
  try {
    if (!pattern.includes('*')) return dirExists(repoPath, pattern);
...
export function detectPlatform(repoPath: string): PlatformProfile {
  ...
  for (const fp of FINGERPRINTS) {
    ...
    for (const f of fp.files ?? []) {
      if (fileExists(repoPath, f)) { hit = true; indicators.push(f); break; }
    }
    for (const d of fp.dirs ?? []) {
      if (!hit && (d.includes('*') ? globExists(repoPath, d) : dirExists(repoPath, d))) {
        hit = true; indicators.push(d);
      }
    }
    if (hit) scores[fp.platform] += fp.weight;
  }
  ...
  if (primary === 'flutter' || primary === 'expo' || primary === 'react-native') {
    if (scores.android > 0) targets.push('android');
    if (scores.ios > 0) targets.push('ios');
  } else {
    targets.push(primary);
  }

  const confidence: PlatformProfile['confidence'] =
    topScore >= 10 ? 'high' : topScore >= 6 ? 'medium' : 'low';

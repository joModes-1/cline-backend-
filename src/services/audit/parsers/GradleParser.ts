...
const FLAGGED_DEPENDENCIES: { pattern: RegExp; title: string; fix: string; severity: 'BLOCKER' | 'WARNING' }[] = [
  {
...
function extractGradleInt(content: string, key: string): number | null {
...
function extractGradleString(content: string, key: string): string | null {
  const m = new RegExp(`${key}\\s*=?\\s*["']([^"']+)["']`).exec(content);
  return m ? m[1] : null;
}

// ─── Main parser ──────────────────────────────────────────────────────────────
export function parseGradle(repoPath: string): ParserResult {

...
function walkFind(dir, target, depth) {
    if (depth === 0)
        return undefined;
...
function extractAttributes(xml, tag, attr) {
    const results = [];
...
function hasAttribute(xml, tag, attr, value) {
    const re = new RegExp(`<${tag}[^>]*${attr}="${value}"[^>]*>`, 'i');
    return re.test(xml);
}
function extractTagAttribute(xml, attr) {

...
function* walkSourceFiles(dir, depth = 0) {
    if (depth > 8)
        return;
    let entries;
    try {
...
function extractManifestPermissions(repoPath) {
    ...
    for (const c of candidates) {
        ...
        while ((m = re.exec(xml)) !== null) {
            if (m[1].startsWith('android.permission.'))
                perms.push(m[1]);
        }
        return perms;
    }
...
function buildSourceCorpus(repoPath) {
    ...
    for (const filePath of walkSourceFiles(repoPath)) {
        if (fileCount > 300)
            break; // cap to avoid memory issues on huge repos
        try {
...
// ─── Main mapper ──────────────────────────────────────────────────────────────
export function mapPermissions(repoPath) {
    const findings = [];
...
//# sourceMappingURL=PermissionMapper.js.map
...
const API_SIGNALS: { category: string; signals: RegExp[] }[] = [
  {
...
const SDK_REQUIRING_PRIVACY_MANIFEST: { name: string; pattern: RegExp }[] = [
...
function findPrivacyManifest(repoPath: string): string | null {
  const candidates = [
    ...
    try {
      ...
      for (const e of entries) {
        if (e.isDirectory()) {
          ...
          if (fs.existsSync(nested)) return path.relative(repoPath, nested);
        }
      }
    } catch { /* skip */ }
  }
...
function buildCorpus(repoPath: string): string {
...
// ─── Main checker ─────────────────────────────────────────────────────────────
export function checkPrivacyManifest(repoPath: string): ParserResult {
  ...
  if (!manifestPath) {
    findings.push({
      ...
      description:
        'Apple requires a PrivacyInfo.xcprivacy manifest for all apps submitted to the App Store (enforced from Spring 2024, Xcode 15+). ' +
        'Apps uploaded without this file are rejected by App Store Connect validation.',
      fixSuggestion:
        'Create PrivacyInfo.xcprivacy in your iOS project root. ' +
    ...
    if (sdksFound.length > 0) {
      findings.push({
        ...
        description:
          `The following third-party SDKs require a PrivacyInfo.xcprivacy manifest: ${sdksFound.map(s => s.name).join(', ')}. ` +
          `Each SDK's privacy manifest must be bundled or merged into your app's manifest.`,
        fixSuggestion:
          'Create PrivacyInfo.xcprivacy. For each SDK listed, include their required API types and data categories. ' +
  ...
  if (!hasAPITypes) {
    ...
    if (triggeredAPIs.length > 0) {
      findings.push({
        ...
        description:
          `Source code uses APIs that require declaration (${triggeredAPIs.map(a => a.category.replace('NSPrivacyAccessedAPICategory', '')).join(', ')}) ` +
          'but NSPrivacyAccessedAPITypes is not declared in PrivacyInfo.xcprivacy.',
        file: manifestPath,
        fixSuggestion:
          'Add NSPrivacyAccessedAPITypes array to PrivacyInfo.xcprivacy with the appropriate category strings for each API your app uses.',

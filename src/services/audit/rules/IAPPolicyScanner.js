...
// ─── Main scanner ─────────────────────────────────────────────────────────────
export function scanIAPPolicy(repoPath) {
    const findings = [];
    ...
    for (const sdk of PAYMENT_SDKS) {
        ...
        if (!foundInSource && !foundInPackage)
            continue;
    ...
    if (!hasNativeIAP && detected.filter(d => d !== 'RevenueCat').length > 0 && isIos) {
        findings.push({
            ...
            description: 'External payment SDKs were found but no native Apple IAP (StoreKit) implementation was detected. ' +
                'If any digital goods or subscriptions are sold, this is a policy violation.',
            fixSuggestion: 'Implement StoreKit or use react-native-iap / RevenueCat to handle iOS purchases natively.',
...
//# sourceMappingURL=IAPPolicyScanner.js.map
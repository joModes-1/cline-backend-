...
function findNetworkSecurityConfig(repoPath: string, manifestXml: string): string | null {
...
function findPlistFiles(repoPath: string): string[] {
...
function findHardcodedHttp(repoPath: string): { file: string; line: number; value: string }[] {
  ...
  function walk(dir: string, depth: number): void {
    if (depth > 7 || results.length > 20) return;
    try {
...
// ─── Main checker ─────────────────────────────────────────────────────────────
export function checkNetworkSecurity(repoPath: string): ParserResult {
  ...
  if (manifestRelPath) {
    ...
    if (/android:usesCleartextTraffic="true"/.test(xml)) {
      findings.push({
        ...
        description:
          'The application globally allows unencrypted HTTP traffic. ' +
        ...
        suggestedFix: {
          description: 'Flip the cleartext-traffic flag off',
          replacement: 'android:usesCleartextTraffic="false"',
        },
      });
    }
      ...
      try {
        ...
        if (/<base-config\s[^>]*cleartextTrafficPermitted="true"/.test(configXml)) {
          findings.push({
            ...
            description:
              'The base-config in network_security_config.xml permits cleartext traffic for all domains. ' +
              'This is flagged by Play Store security review for production builds.',
            file: configRelPath,
            fixSuggestion:
              'Remove cleartextTrafficPermitted="true" from base-config. ' +
              'Only allow cleartext for specific domains under a <domain-config> block in debug builds.',
            suggestedFix: {
              description: 'Disable cleartext at the base-config level',
              replacement: 'cleartextTrafficPermitted="false"',
            },
          });
        }
        ...
        if (!configXml.includes('<pin-set') && !configXml.includes('pin sha256')) {
          findings.push({
            ...
            description:
              'network_security_config.xml exists but no certificate pins are configured. ' +
              'For apps handling sensitive data, certificate pinning prevents MITM attacks.',
            file: configRelPath,
            fixSuggestion:
              'Add a <pin-set> block to network_security_config.xml for your production API domains. ' +
        ...
        if (/<base-config[^>]*>[\s\S]*?<trust-anchors[\s\S]*?<certificates src="user"/.test(configXml)) {
          findings.push({
            ...
            description:
              'network_security_config.xml trusts user-installed CA certificates for all connections. ' +
              'This makes the app vulnerable to MITM attacks and is a hard rejection by Google Play security review.',
            file: configRelPath,
            fixSuggestion:
              'Remove <certificates src="user"/> from the base-config. ' +
    ...
    try {
      ...
      if (/<key>NSAllowsArbitraryLoads<\/key>\s*<true\/>/.test(plist)) {
        findings.push({
          ...
          description:
            'App Transport Security is fully disabled. This allows all HTTP connections without encryption. ' +
          ...
          suggestedFix: {
            description: 'Re-enable ATS by flipping NSAllowsArbitraryLoads to false',
            replacement: '<key>NSAllowsArbitraryLoads</key>\n\t<false/>',
          },
        });
      }
      ...
      if (/<key>NSAllowsArbitraryLoadsInWebContent<\/key>\s*<true\/>/.test(plist)) {
        findings.push({
          ...
          description:
            'ATS is disabled for WebView content. If the app loads untrusted URLs in a WebView, this is a security risk.',
          file: plistPath,
          fixSuggestion:
            'Only disable WebView ATS if your app intentionally loads third-party HTTP content. ' +
  ...
  if (httpUrls.length > 0) {
    ...
    findings.push({
      ...
      description:
        `Hardcoded HTTP (non-HTTPS) URLs were found in source code (e.g. "${first.value}" at ${first.file}:${first.line}). ` +
        'These will fail on Android 9+ (cleartext blocked by default) and trigger ATS violations on iOS.',
      fixSuggestion:
        'Replace all http:// URLs with https://. Use environment variables for API base URLs.',
    });
  }

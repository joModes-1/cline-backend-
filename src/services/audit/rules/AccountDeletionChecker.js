...
const ACCOUNT_CREATION_SIGNALS = [
...
// ─── Main checker ─────────────────────────────────────────────────────────────
export function checkAccountDeletion(repoPath) {
    const findings = [];
    ...
    if (hasCreation && !hasDeletion) {
        findings.push({
            ...
            description: 'This app appears to support account creation but no account deletion mechanism was found. ' +
            ...
            fixSuggestion: 'Add a "Delete Account" option in your app settings or account profile screen. ' +
    ...
    if (!hasCreation && !hasDeletion) {
        findings.push({
            ...
            description: 'No account creation or deletion signals were detected in the scanned source files. ' +
                'If your app supports user accounts, manually verify that account deletion is implemented.',
            fixSuggestion: 'If your app has user accounts, add a visible "Delete Account" option that removes all user data. Required by Apple and Google.',
        });
    }
    return {
...
//# sourceMappingURL=AccountDeletionChecker.js.map
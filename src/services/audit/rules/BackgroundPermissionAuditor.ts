...
// ─── High-risk background permissions with deep justification analysis ────────
// These go beyond ManifestParser's basic flag — this auditor cross-checks
// whether the app can justify these to the Play Store Permissions Declaration.

interface BackgroundPermissionRule {
  permission: string;
  label: string;
  severity: 'BLOCKER' | 'WARNING';
  requiresDeclaration: boolean;    // needs a Play Console Permissions Declaration Form
  justificationSignals: RegExp[];  // source patterns that justify this permission
  rejectionRisk: string;
  fix: string;
  storeRule?: string;
}

const BACKGROUND_RULES: BackgroundPermissionRule[] = [
  {
    ...
    rejectionRisk:
      'Google Play manually reviews all apps requesting background location. ' +
      'Without a valid core feature requiring it (navigation, fitness tracking, geofencing), this is rejected.',
    fix:
      'Remove ACCESS_BACKGROUND_LOCATION unless your core feature requires it. ' +
  ...
  },
  {
    ...
    rejectionRisk:
      'Foreground services are scrutinized by Play Store reviewers for battery abuse and background data collection.',
    fix:
      'Declare a foregroundServiceType in the <service> tag. ' +
  ...
  },
  {
    ...
    rejectionRisk:
      'Boot receivers that silently start background activity are flagged for battery / resource abuse by Play Store scanners.',
    fix:
      'Ensure the boot receiver only schedules lightweight background tasks (WorkManager jobs). ' +
  ...
  },
  {
    ...
    rejectionRisk:
      'Apps requesting REQUEST_INSTALL_PACKAGES that are not device management, enterprise MDM, or app stores are rejected outright.',
    fix:
      'Remove REQUEST_INSTALL_PACKAGES unless your app is explicitly an APK installer, MDM agent, or enterprise tool. ' +
  ...
  },
  {
    ...
    rejectionRisk:
      'SYSTEM_ALERT_WINDOW (draw over other apps) requires Google approval. ' +
      'Only productivity, accessibility, and bubble-notification apps qualify. Consumer apps are rejected.',
    fix:
      'Remove SYSTEM_ALERT_WINDOW unless your app requires overlay functionality as a core feature. ' +
  ...
  },
  {
    ...
    rejectionRisk:
      'MANAGE_EXTERNAL_STORAGE (All Files Access) is restricted to file managers, AV scanners, and backup apps. ' +
      'General apps requesting this are rejected. Google manually reviews each submission.',
    fix:
      'Replace MANAGE_EXTERNAL_STORAGE with scoped storage APIs (MediaStore, Storage Access Framework). ' +
...
function hasPermission(xml: string, permission: string): boolean {
    ...
    try {
      for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (e.isDirectory()) walk(full, depth + 1);
          ...
          try { corpus += fs.readFileSync(full, 'utf-8') + '\n'; count++; } catch { /* skip */ }
        }
      }
    } catch { /* skip */ }
  }
...
// ─── Main auditor ─────────────────────────────────────────────────────────────
export function auditBackgroundPermissions(repoPath: string): ParserResult {
  ...
  for (const rule of BACKGROUND_RULES) {
    if (!hasPermission(xml, rule.permission)) continue;

    const isJustified = rule.justificationSignals.some(re => re.test(corpus));

    if (!isJustified) {
      flagged.push(rule.label);
      if (rule.requiresDeclaration) needsDeclaration.push(rule.label);
      ...
      findings.push({
        ...
        description:
          `${rule.permission} is declared in AndroidManifest.xml but no corresponding usage was found in source code. ` +
          rule.rejectionRisk,
      ...
      if (rule.requiresDeclaration) {
        ...
        findings.push({
          ...
          description:
            `${rule.permission} is declared and appears to be actively used. ` +
          ...
          value: rule.permission,
          fixSuggestion:
            'In Google Play Console → App Content → Permissions Declaration, submit a detailed explanation of why this permission is required for core app functionality.',
          storeRule: rule.storeRule,
        });
      }
    }

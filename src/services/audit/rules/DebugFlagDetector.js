...
const DEBUG_CHECKS = [
    // ── Android Manifest ────────────────────────────────────────────────────────
    {
        ...
        severity: 'BLOCKER',
        description: 'android:debuggable="true" is present in the production manifest. ' +
        ...
        fix: 'Remove android:debuggable="true" from AndroidManifest.xml. ' +
    ...
    },
    {
        ...
        severity: 'BLOCKER',
        description: 'android:testOnly="true" prevents the app from being installed via the Play Store. ' +
    ...
    // ── Gradle build config ─────────────────────────────────────────────────────
    {
        ...
        severity: 'BLOCKER',
        description: 'The release build variant is configured to use the debug signing certificate. ' +
            'Apps signed with debug keys cannot be uploaded to Google Play and will fail in production.',
        fix: 'Configure a production signing key in Gradle. ' +
    ...
    },
    {
        ...
        severity: 'WARNING',
        description: 'minifyEnabled false in the release build disables R8/ProGuard code shrinking and obfuscation. ' +
    ...
    // ── Environment / config files ──────────────────────────────────────────────
    {
        ...
        excludePattern: /^\s*#/,
        severity: 'WARNING',
        description: 'DEBUG=true found in a committed .env file. This enables verbose logging, stack traces, and ' +
            'development endpoints in production, exposing sensitive information to users.',
        fix: 'Set DEBUG=false in production .env files. ' +
    ...
    },
    {
        ...
        severity: 'WARNING',
        description: 'NODE_ENV=development in a production config file disables production optimizations, ' +
    ...
    // ── iOS / Xcode ──────────────────────────────────────────────────────────────
    {
        ...
        severity: 'WARNING',
        description: 'Flipper (Facebook debug tool) is enabled in the Podfile. Shipping Flipper in a production build ' +
            'increases app size, may cause crashes on non-debug devices, and exposes internal network traffic.',
        fix: "Wrap Flipper in a debug-only condition:\n" +
    ...
    // ── JavaScript / React Native ────────────────────────────────────────────────
    {
        ...
        severity: 'INFO',
        description: 'console.warn/error is suppressed globally. This can hide critical runtime errors from crash reporters.',
    ...
    },
    {
        ...
        severity: 'WARNING',
        description: 'React DevTools connection code found in source. Shipping this in production exposes app state to connected debuggers.',
...
// ─── File reader helper ───────────────────────────────────────────────────────
function readFile(repoPath, relPaths) {
...
function buildBuildCorpus(repoPath) {
    const BUILD_FILES = [
    ...
    return BUILD_FILES
        ...
        .map(f => {
        try {
            return { content: fs.readFileSync(path.join(repoPath, f), 'utf-8'), file: f };
        }
        catch {
            return null;
        }
...
// ─── Main detector ────────────────────────────────────────────────────────────
export function detectDebugFlags(repoPath) {
    const findings = [];
    ...
    for (const check of DEBUG_CHECKS) {
        ...
        if (!check.contentPattern.test(content))
            continue;
        if (check.excludePattern?.test(content))
            continue;
        findings.push({
            id: check.id,
    ...
    if (fs.existsSync(gitignorePath)) {
        try {
            ...
            if (!gitignore.includes('.env') && (fs.existsSync(path.join(repoPath, '.env')) ||
                ...
                findings.push({
                    ...
                    description: '.env file(s) exist in the repo but .gitignore does not exclude them. ' +
...
//# sourceMappingURL=DebugFlagDetector.js.map
...
// ─── Patterns that indicate thin / placeholder content ────────────────────────
...
function* walkFiles(dir: string, maxDepth = 8, depth = 0): Generator<string> {
...
// ─── Main scanner ─────────────────────────────────────────────────────────────
export function scanPlaceholders(repoPath: string): ParserResult {
  const findings: AuditFinding[] = [];
  const seenIds = new Set<string>(); // deduplicate same finding across files (cap at 3 per pattern)
  ...
  for (const filePath of walkFiles(repoPath)) {
    ...
    for (const pattern of PLACEHOLDER_PATTERNS) {
      ...
      if (patternHits[pattern.id] >= 5) continue; // cap findings per pattern

...
function calculateScore(findings, platformFilter) {
    let score = 100;
    for (const f of findings) {
        if (platformFilter && f.platform !== platformFilter && f.platform !== 'both')
            continue;
...
export async function runAudit(config) {
    ...
    const parserResults = [];
    const run = (name) => !skipParsers.includes(name);
    ...
    // ── Layer 1: Static Parsers ───────────────────────────────────────────────
    if (run('PlaceholderScanner'))
        parserResults.push(scanPlaceholders(repoPath));
    if (hasAndroid || isUnknown) {
        if (run('ManifestParser'))
            parserResults.push(parseManifest(repoPath));
        if (run('GradleParser'))
            parserResults.push(parseGradle(repoPath));
        if (run('PermissionMapper'))
            parserResults.push(mapPermissions(repoPath));
    }
    if (hasIos || isUnknown) {
        if (run('PlistParser'))
            parserResults.push(parsePlist(repoPath));
    }
    if (isFlutter || isUnknown) {
        if (run('PubspecParser'))
            parserResults.push(parsePubspec(repoPath));
    }
    if (isJs || isUnknown) {
        if (run('PackageJsonAudit'))
            parserResults.push(auditPackageJson(repoPath));
    }
    // ── Layer 2: Compliance Rules (always run — platform-agnostic where needed) ─
    if (run('AccountDeletionChecker'))
        parserResults.push(checkAccountDeletion(repoPath));
    if (run('DebugFlagDetector'))
        parserResults.push(detectDebugFlags(repoPath));
    if (run('NetworkSecurityChecker'))
        parserResults.push(checkNetworkSecurity(repoPath));
    if (hasAndroid || isUnknown) {
        if (run('ApiLevelGapDetector'))
            parserResults.push(detectApiLevelGaps(repoPath));
        if (run('BackgroundPermissionAuditor'))
            parserResults.push(auditBackgroundPermissions(repoPath));
    }
    if (hasIos || isUnknown) {
        if (run('IAPPolicyScanner'))
            parserResults.push(scanIAPPolicy(repoPath));
        if (run('PrivacyManifestChecker'))
            parserResults.push(checkPrivacyManifest(repoPath));
    }
    // ── Aggregate + deduplicate findings ─────────────────────────────────────
    const seen = new Set();
    const dedupedFindings = parserResults
        .flatMap(r => r.findings)
        .filter(f => { if (seen.has(f.id))
        return false; seen.add(f.id); return true; });
    const sortOrder = { BLOCKER: 0, WARNING: 1, INFO: 2 };
    dedupedFindings.sort((a, b) => sortOrder[a.severity] - sortOrder[b.severity]);
    // ── Scores ────────────────────────────────────────────────────────────────
    const scores = { overall: calculateScore(dedupedFindings) };
    if (hasAndroid)
        scores.android = calculateScore(dedupedFindings, 'android');
    if (hasIos)
        scores.ios = calculateScore(dedupedFindings, 'ios');

...
export interface CodeReviewIssue {
  ...
  ruleId: string;
  source: string;
  // Compliance-reviewer fields. Optional so the autofix path (which only
  // reads file/message/suggestedFix.replacement) is unaffected.
...
export class VillaCodeReviewService {
  ...
  constructor(controller: Controller, stateManager: StateManager) {
   ...
   */
  async reviewFile(options: CodeReviewOptions): Promise<CodeReviewResult> {
    ...
    const systemPrompt = `You are a Pre-Submission Compliance Reviewer for the Apple App Store and the
Google Play Store. Your job is to read ONE source file from an app and flag
anything that would cause the app to be REJECTED by store reviewers, OR that
is a security/runtime risk that must be fixed before release.

Return ONLY a JSON array of issue objects with this shape:
{
  "message": "What is wrong, specifically.",
  "severity": "error" | "warning" | "info",
  "line": <1-indexed line number where the issue lives>,
  "column": <1-indexed column, or 1>,
  "ruleId": "<category-prefix>/<kebab-case-rule>",
  "category": "SECURITY" | "COMPLIANCE" | "PERMISSIONS" | "CONTENT" | "QUALITY" | "CONFIGURATION",
  "storeRule": "<App Store / Play Store policy reference when relevant>",
  "suggestedFix": {
    "description": "What change fixes this",
    "replacement": "<verbatim replacement text for the affected line(s)>"
  }
}

WHAT TO FLAG (priority order):

[SECURITY]  Hardcoded API keys, tokens, JWT secrets, DB passwords, OAuth
            client secrets, private keys, .env values committed inline.
            Plain strings used where process.env.X belongs.

[COMPLIANCE] Stripe/PayPal/external payment SDK for digital goods in iOS
             context (App Store Guideline 3.1.1).
             Cleartext http:// in fetch/axios production paths.
             Code enabling android:usesCleartextTraffic or android:debuggable
             in release builds.
             User-account code with no account-deletion handler (Apple
             mandate, June 2022).
             Firebase Analytics / Facebook SDK / ad SDKs without an App
             Tracking Transparency request (iOS 14.5+).

[PERMISSIONS] Code that calls privileged APIs (location, camera, mic,
              contacts, background) without a clear user-facing trigger
              visible in the same file.

[CONTENT]   "Lorem ipsum", "TODO", "Coming Soon", "test@test.com",
            "example.com", placeholder strings, debug console.log() left
            in production code paths.

[QUALITY]   Crash-prone runtime code: uncaught throws, missing await,
            undefined property access on user input, SQL injection
            (string concatenation), XSS (innerHTML of untrusted input).

[CONFIGURATION] Hardcoded production URLs in dev paths, debug flags in
                release, missing engines.node, missing main entry.

RULES:
- Return ONLY the JSON array. No prose, no markdown fence.
- Empty file or nothing found -> return [].
- Every issue MUST include "line" pointing at the actual line in this file.
- suggestedFix.replacement MUST be verbatim text that can replace the
  flagged line(s) byte-for-byte. If you cannot produce one, OMIT
  suggestedFix for that issue.
- Do NOT invent permissions, SDKs, or features not present in the file.
- Do NOT flag style preferences. Only real rejection / security / crash risks.
- You see ONE file at a time. If a check requires another file (e.g.
  cross-reference with manifest), only flag if the evidence is in THIS file.`;

    const userPrompt = `File path: ${options.fileName}\nLanguage: ${detectedLanguage}\n\nContent:\n\n\`\`\`${detectedLanguage}\n${content}\n\`\`\``;

    try {
      Logger.info('[VillaCodeReview] Sending request to AI...');
      Logger.info(`[VillaCodeReview] Provider: ${(apiConfig as any).actModeApiProvider || 'default'}`);
      ...
      issues = issues.filter((issue) => {
        ...
        if (typeof replacement !== 'string' || replacement.length === 0) return true;
        ...
        const issueCode = (issue as unknown as { code?: unknown }).code;
   ...
   */
  async scanRepository(options: RepositoryScanOptions): Promise<RepositoryScanResult> {
    const startTime = Date.now();
    ...
    // Review each file
    const results: RepositoryScanResult['files'] = [];
    let totalIssues = 0;
    let totalErrors = 0;
    let totalWarnings = 0;
   ...
   */
  async applyFix(filePath: string, fix: CodeReviewIssue['suggestedFix']): Promise<boolean> {
    if (!fix) {
      Logger.warn('[VillaCodeReview] No fix provided');
      return false;
    }

		const start = (fix as any)?.range?.start
		const end = (fix as any)?.range?.end
   ...
   */
  private async getCachedReview(fileName: string, content: string): Promise<CodeReviewResult | null> {
    try {
      ...
      const cached: CodeReviewResult = JSON.parse(data);
   ...
   * Cache review result
   */
  private async cacheReview(fileName: string, content: string, result: CodeReviewResult): Promise<void> {
    try {
  ...
  private async findFiles(
    ...
    const isHardExcluded = (fullPath: string): boolean => {
      ...
      if (excludedDirs.some((d) => p.includes(d))) return true
    ...
    const matchesPatterns = (fullPath: string): boolean => {
      const normalizedFullPath = fullPath.replace(/\\/g, "/")

      if (isHardExcluded(normalizedFullPath)) return false
    ...
    const walk = async (dir: string): Promise<void> => {
      ...
      for (const entry of entries) {
        ...
        if (isHardExcluded(fullPath)) continue
    ...
    if (files.length > 0) {
      Logger.info(
        `[VillaCodeReview] Sample matched files: ${files
          .slice(0, 10)
  ...
  private isEffectivelyOnlyCommentsOrWhitespace(content: string, language: string): boolean {

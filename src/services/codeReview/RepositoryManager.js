...
export class RepositoryManager {
     ...
     */
    async moveDirectoryContents(srcDir, destDir) {
     ...
     */
    async flattenSingleTopLevelFolder(repoRoot) {
        try {
     ...
     */
    async cloneGitHubRepo(repoInfo) {
        ...
        try {
            ...
            if (process.platform === 'win32') {
                await execAsync(`powershell -command "Expand-Archive -Path '${zipPath}' -DestinationPath '${localPath}' -Force"`, { timeout: 60000 });
            }
            else {
                await fs.mkdir(localPath, { recursive: true });
                await execAsync(`unzip -q "${zipPath}" -d "${localPath}"`, { timeout: 60000 });
            }
            // Clean up zip file
     ...
     */
    async extractZipFile(zipBuffer, originalName) {
     ...
     */
    async getRepository(repoId, localPath) {
     ...
     */
    async scanRepository(localPath) {
     ...
     */
    async listAllFiles(localPath) {
        Logger.info(`[RepositoryManager] Listing all repository files: ${localPath}`);
        try {
            await fs.access(localPath);
        }
        catch (e) {
            Logger.warn(`[RepositoryManager] Repo path not accessible: ${localPath}`, e);
            return [];
        }
        await this.flattenSingleTopLevelFolder(localPath);
        const files = [];
     ...
     */
    async writeFileContent(localPath, filePath, content) {
     ...
     */
    async getRepositoryStats(localPath) {
        ...
        for (const file of files) {
            ...
            if (!lineCountExtensions.has(ext))
                continue;
            try {

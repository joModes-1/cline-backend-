...
// Schemas from MCP SDK - using any type due to SDK API changes
const CallToolResultSchema = {} as any
const GetPromptResultSchema = {} as any
const ListPromptsResultSchema = {} as any
const ListResourcesResultSchema = {} as any
const ListResourceTemplatesResultSchema = {} as any
const ListToolsResultSchema = {} as any
const ReadResourceResultSchema = {} as any
export class McpHub {
		...
		try {
			...
			switch (expandedConfig.type) {
				case "stdio": {
					...
					if (!transport || typeof (transport as any).start !== "function") {
						throw new Error(
							`Failed to initialize MCP stdio transport for "${name}". ` +
								`Expected a transport with a start() method, but got: ${transport === undefined ? "undefined" : typeof transport}`,
						)
					}
					...
					const reconnectingEventSourceOptions = {
						...
						fetch: authProvider
							? async (url: string | URL, init?: RequestInit) => {
									const tokens = await (authProvider as any).tokens() // Dynamic - gets fresh tokens
			...
			try {
				...
				;(connection.client as any).setNotificationHandler?.(NotificationMessageSchema as any, async (notification: any) => {
				...
				;(connection.client as any).fallbackNotificationHandler = async (notification: any) => {
	...
	private async fetchToolsList(serverName: string): Promise<McpTool[]> {
		try {
			...
			const response = await (connection.client as any).request({ method: "tools/list" }, ListToolsResultSchema, {
	...
	private async fetchResourcesList(serverName: string): Promise<McpResource[]> {
		try {
			...
			const response = await (connection.client as any).request({ method: "resources/list" }, ListResourcesResultSchema, {
				timeout: DEFAULT_REQUEST_TIMEOUT_MS,
			})
	...
	private async fetchResourceTemplatesList(serverName: string): Promise<McpResourceTemplate[]> {
		try {
			...
			const response = await (connection.client as any).request(
	...
	private async fetchPromptsList(serverName: string): Promise<McpPrompt[]> {
		try {
			...
			const response = await (connection.client as any).request({ method: "prompts/list" }, ListPromptsResultSchema, {
				timeout: DEFAULT_REQUEST_TIMEOUT_MS,
			})
	...
	async readResource(serverName: string, uri: string): Promise<McpResourceResponse> {
		...
		return await (connection.client as any).request(
			{
	...
	async callTool(
		...
		try {
			const result = await (connection.client as any).request(
				{
					method: "tools/call",
					params: {
						name: toolName,
	...
	async completeOAuth(serverHash: string, code: string, state: string | null): Promise<void> {
		...
		if (connection.transport instanceof SSEClientTransport || connection.transport instanceof StreamableHTTPClientTransport) {
			await connection.transport.close?.()
			// For transports with finishAuth method, we don't need to do anything special
			// The SDK handles the token exchange internally
		} else {

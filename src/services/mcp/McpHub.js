...
const GetPromptResultSchema = {};
const ListPromptsResultSchema = {};
...
export class McpHub {
    ...
    async connectToServer(name, config, source) {
        ...
        if (config.type === "stdio") {
            ...
            if (Object.keys(remoteConfig).length === 0) {
                ...
                if (hasAllowlist) {
                    const allowedIds = remoteConfig.allowedMCPServers.map((server) => server.id);
            ...
            switch (expandedConfig.type) {
                case "stdio": {
                    ...
                    if (!transport || typeof transport.start !== "function") {
                        throw new Error(`Failed to initialize MCP stdio transport for "${name}". ` +
                ...
                case "streamableHttp": {
                    ...
                    const reconnectHandler = new StreamableHttpReconnectHandler(name, {
                        ...
                        appendErrorMessage: (conn, msg) => this.appendErrorMessage(conn, msg),
    ...
    async fetchPromptsList(serverName) {
        try {
            const connection = this.connections.find((conn) => conn.server.name === serverName);
     ...
     */
    configsRequireRestart(oldConfig, newConfig) {
        // Exclude Cline-specific settings from comparison (add new ones here)
        const { autoApprove: _oldAutoApprove, timeout: _oldTimeout, remoteConfigured: _oldRemoteConfigured, ...oldConnectionConfig } = oldConfig;
        const { autoApprove: _newAutoApprove, timeout: _newTimeout, remoteConfigured: _newRemoteConfigured, ...newConnectionConfig } = newConfig;
    ...
    async getPrompt(serverName, promptName, promptArguments) {
        const connection = this.connections.find((conn) => conn.server.name === serverName);
        if (!connection) {
            throw new Error(`No connection found for server: ${serverName}`);
        }

...
import { addMcpServerShortcut } from "./utils/mcp";
...
async function disposeTelemetryServices() {
...
function normalizeReasoningEffort(value) {
    ...
    printWarning(`Invalid --reasoning-effort '${value}'. Using 'medium'. Valid values: ${OPENAI_REASONING_EFFORT_OPTIONS.join(", ")}.`);
    return "medium";
}
function normalizeMaxConsecutiveMistakes(value) {
    if (value === undefined) {
        return undefined;
    }
 ...
 */
function applyTaskOptions(options) {
    // Apply mode flag
    if (options.plan) {
    ...
    if (options.model) {
        ...
        const currentProvider = StateManager.get().getGlobalSettingsKey(providerKey);
 ...
 */
function getModeSelection(options) {
...
async function addMcpServer(name, targetOrCommand = [], options) {
    try {
 ...
 */
async function runTaskInPlainTextMode(ctx, options, taskConfig) {
 ...
 */
function createInkCleanup(ctx, onTaskError) {
    return async () => {
        await disposeCliContext(ctx);
        if (onTaskError?.()) {
...
export async function captureUnhandledException(reason, context) {
    try {
...
function onUnhandledException(reason, context) {
 ...
 */
async function initializeCli(options) {
    ...
    const logToChannel = (message) => outputChannel.appendLine(message);
    ...
    outputChannel.appendLine(`Cline CLI initialized. Data dir: ${DATA_DIR}, Extension dir: ${EXTENSION_DIR}, Log dir: ${CLINE_CLI_DIR.log}`);
    HostProvider.initialize(() => new CliWebviewProvider(extensionContext), () => new FileEditProvider(), () => new CliCommentReviewController(), () => new StandaloneTerminalManager(), createCliHostBridgeProvider(workspacePath), logToChannel, async (path) => (options.enableAuth ? AuthHandler.getInstance().getCallbackUrl(path) : ""), getCliBinaryPath, EXTENSION_DIR, DATA_DIR);
    await StateManager.initialize(storageContext);
    await ErrorService.initialize();
    // Create a controller directly instead of using webview provider
    const controller = new Controller(extensionContext);
 ...
 */
async function runInkApp(element, cleanup) {
 ...
 */
async function runTask(prompt, options, existingContext) {
    ...
    let taskError = false;
    await runInkApp(React.createElement(App, {
        view: "welcome",
        verbose: options.verbose,
        controller: ctx.controller,
...
async function showConfig(options) {
    ...
    await runInkApp(React.createElement(ConfigViewWrapper, {
        controller: ctx.controller,
        dataDir: ctx.dataDir,
 ...
 */
async function performQuickAuthSetup(ctx, options) {
 ...
 */
function findTaskInHistory(taskId) {
    const taskHistory = StateManager.get().getGlobalStateKey("taskHistory") || [];
 ...
 */
async function resumeTask(taskId, options, existingContext) {
    const ctx = existingContext || (await initializeCli({ ...options, enableAuth: true }));

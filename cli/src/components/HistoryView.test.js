...
describe("HistoryView", () => {
    ...
    describe("rendering", () => {
        it("should render the history header", () => {
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: mockItems }));
        ...
        it("should show empty message when no items", () => {
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: [] }));
    ...
    describe("task details", () => {
        it("should display task cost when available", () => {
            ...
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: itemsWithCost }));
        ...
        it("should display model ID when available", () => {
            ...
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: itemsWithModel }));
        ...
        it("should truncate long task descriptions", () => {
            ...
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: itemsWithLongTask }));
            expect(lastFrame()).toContain("...");
        });
        it("should handle missing task text", () => {
            const itemsWithoutTask = [{ id: "task-1", ts: Date.now() }];
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: itemsWithoutTask }));
    ...
    describe("keyboard navigation", () => {
        it("should navigate down with arrow key", () => {
            const { lastFrame, stdin } = render(React.createElement(HistoryView, { controller: mockController, items: mockItems }));
            // Press down arrow
            stdin.write("\x1B[B");
    ...
    describe("pagination", () => {
        it("should show pagination info when provided", () => {
            ...
            const { lastFrame } = render(React.createElement(HistoryView, { controller: mockController, items: mockItems, pagination: pagination }));
    ...
    describe("scrolling", () => {
        it("should show scroll indicators for long lists", () => {
            ...
            const { lastFrame, stdin } = render(React.createElement(HistoryView, { controller: mockController, items: manyItems, visibleCount: 5 }));
            // Navigate down a bit
            for (let i = 0; i < 5; i++) {
                stdin.write("\x1B[B");
            }
...
//# sourceMappingURL=HistoryView.test.js.map
...
import { checkAndWarnRipgrepMissing, extractMentionQuery, insertMention, searchWorkspaceFiles, } from "../utils/file-search";
import { isMouseEscapeSequence } from "../utils/input";
import { jsonParseSafe, parseImagesFromInput } from "../utils/parser";
import { createCliOnlySlashCommands, extractSlashQuery, filterCommands, getStandaloneSlashCommandToExecute, insertSlashCommand, sortCommandsWorkflowsFirst, } from "../utils/slash-commands";
...
import { ActionButtons, getButtonConfig, getVisibleButtons } from "./ActionButtons";
...
const inputStateStorage = new Map();
function getInputStorageKey(controller, taskId) {
 ...
 */
const YOLO_INTERACTIVE_ASKS = new Set([
    "completion_result",
...
function isYoloSuppressed(yolo, ask) {
    return yolo && (!ask || !YOLO_INTERACTIVE_ASKS.has(ask));
}
/**
 * Get the type of prompt needed for an ask message
 */
function getAskPromptType(ask, text) {
    switch (ask) {
        case "followup":
        case "plan_mode_respond": {
            const parts = jsonParseSafe(text, { options: undefined });
 ...
 */
function expandPastedTexts(text, pastedTexts) {
    return text.replace(/\[Pasted text #(\d+) \+\d+ lines\]/g, (match, num) => {
        const content = pastedTexts.get(Number.parseInt(num, 10));
        return content ?? match;
    });
}
export const ChatView = ({ controller, onExit, onComplete: _onComplete, onError, initialPrompt, initialImages, taskId, }) => {
    ...
    const { text: textInput, cursorPos, setText: setTextInput, setCursorPos, handleKeyboardSequence, handleCtrlShortcut, deleteCharBefore, insertText: insertTextAtCursor, } = useTextInput();
    ...
    const [respondedToAsk, setRespondedToAsk] = useState(null);
    ...
    const activePasteNumRef = useRef(0);
    const activePasteStartPosRef = useRef(0); // Where the placeholder starts in the text
    const activePasteLinesRef = useRef(0); // Total line count for current paste
    const pasteUpdateTimeoutRef = useRef(null); // Debounce placeholder updates
    ...
    // Slash command state
    const [availableCommands, setAvailableCommands] = useState(() => createCliOnlySlashCommands());
    const [selectedSlashIndex, setSelectedSlashIndex] = useState(0);
    const [slashMenuDismissed, setSlashMenuDismissed] = useState(false);
    const lastSlashIndexRef = useRef(-1);
    // Panel state
    ...
    const prevFirstMessageTs = useRef(null);
    ...
    const [gitDiffStats, setGitDiffStats] = useState(null);
    // Mode state
    const [mode, setMode] = useState(() => {
        const stateManager = StateManager.get();
        return stateManager.getGlobalSettingsKey("mode") || "act";
    });
    const [yolo, _setYolo] = useState(() => StateManager.get().getGlobalSettingsKey("yoloModeToggled") ?? false);
    const [autoApproveAll, setAutoApproveAll] = useState(() => StateManager.get().getGlobalSettingsKey("autoApproveAllToggled") ?? false);
    // Sync mode from core state updates (e.g. yolo auto-switching plan to act)
    useEffect(() => {
        if (taskState.mode && taskState.mode !== mode) {
            setMode(taskState.mode);
        }
    }, [taskState.mode]);
    ...
    const provider = useMemo(() => {
        ...
        return stateManager.getGlobalSettingsKey(providerKey) || "";
    }, [mode, activePanel]);
    ...
    const modelId = useMemo(() => {
        ...
        return stateManager.getGlobalSettingsKey(modelKey) || getProviderDefaultModelId(provider) || "";
    ...
    const filteredCommands = useMemo(() => filterCommands(availableCommands, slashInfo.query), [availableCommands, slashInfo.query]);
    ...
    const displayMessages = useMemo(() => {
        const filtered = messages.filter((m) => {
            ...
            if (m.say === "api_req_started")
                return false;
            if (m.say === "api_req_retried")
                return false; // Redundant with error_retry messages
            if (m.say === "reasoning")
                return false; // Hide thinking traces - they clutter the UI
            return true;
        });
    ...
    const { completedMessages, currentMessage } = useMemo(() => {
        ...
        const isUnselectedFollowup = (msg) => {
        ...
        const isFileEditToolMessage = (msg) => {
        ...
        const shouldCommandStayInDynamic = (msg, isLast) => {
        ...
        for (let i = 0; i < displayMessages.length; i++) {
            ...
            const shouldSkipDynamic = skipDynamicTypes.has(msg.say || "") ||
    ...
    const pendingAsk = lastMessage?.type === "ask" && !lastMessage.partial && respondedToAsk !== lastMessage.ts ? lastMessage : null;
    const askType = pendingAsk ? getAskPromptType(pendingAsk.ask, pendingAsk.text || "") : "none";
    const askOptions = pendingAsk && askType === "options" ? parseAskOptions(pendingAsk.text || "") : [];
    // Send response to ask message
    const sendAskResponse = useCallback(async (responseType, text) => {
        if (!ctrl?.task || !pendingAsk)
            return;
    ...
    const handleCliOnlySlashCommand = useCallback((commandName) => {
        if (commandName === "help") {
    ...
    const handleButtonAction = useCallback(async (action, _isPrimary) => {
        if (!action)
            return;
        if (!ctrl)
            return;
    ...
    const handleSubmit = useCallback(async (text, images) => {
        ...
        try {
            ...
            const imageDataUrls = images.length > 0
    ...
    useInput((input, key) => {
        ...
        if (pendingAsk && !isYoloSuppressed(yolo, pendingAsk.ask)) {
    ...
    return (React.createElement(Box, { flexDirection: "column", key: taskSwitchKey, width: "100%" },
        React.createElement(Static, { items: staticItems }, (item) => {
            if (item.type === "header") {
                // Show static robot frame in header (first frame, looking straight ahead)
                return (React.createElement(Box, { flexDirection: "column", key: "header" },
                    React.createElement(StaticRobotFrame, null),
                    React.createElement(Text, null, " "),
                    React.createElement(Text, { bold: true, color: "white" }, centerText("What can I do for you?")),
                    React.createElement(Text, null, " ")));
            }
            // Completed message
            return (React.createElement(Box, { key: item.message.ts, paddingX: 1, width: "100%" },
                React.createElement(ChatMessage, { message: item.message, mode: mode })));
        }),
        React.createElement(Box, { flexDirection: "column", width: "100%" },
            isWelcomeState && (React.createElement(Box, { flexDirection: "column", marginBottom: 1 },
                React.createElement(AsciiMotionCli, { onInteraction: () => setUserScrolled(true) }),
                React.createElement(Text, null, " "),
                React.createElement(Text, { bold: true, color: "white" }, centerText("What can I do for you?")))),
            currentMessage && (React.createElement(Box, { paddingX: 1, width: "100%" },
                React.createElement(ChatMessage, { isStreaming: true, message: currentMessage, mode: mode }))),
            buttonConfig.enableButtons &&
                !isSpinnerActive &&
                !isYoloSuppressed(yolo, pendingAsk?.ask) && (React.createElement(ActionButtons, { config: buttonConfig, mode: mode })),
            isSpinnerActive && React.createElement(ThinkingIndicator, { mode: mode, onCancel: handleCancel, startTime: spinnerStartTime }),
            !activePanel && !isExiting && (React.createElement(Box, { borderColor: borderColor, borderStyle: "round", flexDirection: "row", justifyContent: "space-between", paddingLeft: 1, paddingRight: 1, width: "100%" },
                React.createElement(Box, null,
                    inputPrompt && React.createElement(Text, { color: borderColor },
                        inputPrompt,
                        " "),
                    React.createElement(HighlightedInput, { availableCommands: availableCommands.map((c) => c.name), cursorPos: cursorPos, text: textInput })))),
            activePanel?.type === "settings" && (React.createElement(SettingsPanelContent, { controller: ctrl, initialMode: activePanel.initialMode, initialModelKey: activePanel.initialModelKey, onClose: () => setActivePanel(null) })),
            activePanel?.type === "history" && ctrl && (React.createElement(HistoryPanelContent, { controller: ctrl, onClose: () => setActivePanel(null), onSelectTask: () => setActivePanel(null) })),
            activePanel?.type === "help" && React.createElement(HelpPanelContent, { onClose: () => setActivePanel(null) }),
            activePanel?.type === "skills" && ctrl && (React.createElement(SkillsPanelContent, { controller: ctrl, onClose: () => setActivePanel(null), onUseSkill: (skillPath) => {
                    setActivePanel(null);
                    setTextInput(`@${skillPath} `);
                    setCursorPos(skillPath.length + 2);
                } })),
            showSlashMenu && !activePanel && (React.createElement(Box, { paddingLeft: 1, paddingRight: 1 },
                React.createElement(SlashCommandMenu, { commands: filteredCommands, query: slashInfo.query, selectedIndex: selectedSlashIndex }))),
            showFileMenu && !activePanel && (React.createElement(Box, { paddingLeft: 1, paddingRight: 1 },
                React.createElement(FileMentionMenu, { isLoading: isSearching, query: mentionInfo.query, results: fileResults, selectedIndex: selectedIndex, showRipgrepWarning: showRipgrepWarning }))),
            imagePaths.length > 0 && !activePanel && (React.createElement(Box, { paddingLeft: 1, paddingRight: 1 },
                React.createElement(Text, { color: "magenta" },
                    imagePaths.length,
                    " image",
                    imagePaths.length > 1 ? "s" : "",
                    " attached"))),
            !showSlashMenu && !showFileMenu && !activePanel && (React.createElement(Box, { flexDirection: "column", width: "100%" },
                React.createElement(Box, { justifyContent: "space-between", paddingLeft: 1, paddingRight: 1, width: "100%" },
                    React.createElement(Box, { flexShrink: 1, flexWrap: "wrap" },
                        React.createElement(Text, { color: "gray" }, "/ for commands \u00B7 @ for files")),
                    React.createElement(Box, { flexShrink: 0, gap: 1 },
                        React.createElement(Box, null,
                            React.createElement(Text, { bold: mode === "plan", color: mode === "plan" ? "yellow" : undefined },
                                mode === "plan" ? "●" : "○",
                                " Plan")),
                        React.createElement(Box, null,
                            React.createElement(Text, { bold: mode === "act", color: mode === "act" ? COLORS.primaryBlue : undefined },
                                mode === "act" ? "●" : "○",
                                " Act")),
                    ...
                    React.createElement(Text, null,
                        ...
                        (() => {
                            ...
                            return (React.createElement(Text, null,
                                React.createElement(Text, null, bar.filled),
                                React.createElement(Text, { color: "gray" }, bar.empty)));
                        })(),
                        " ",
                ...
                React.createElement(Box, { paddingLeft: 1, paddingRight: 1 },
                    React.createElement(Text, null,
                        ...
                        gitBranch && ` (${gitBranch})`,
                        gitDiffStats && gitDiffStats.files > 0 && (React.createElement(Text, { color: "gray" },
                            " ",
                            "| ",
                            gitDiffStats.files,
                            " file",
                ...
                React.createElement(Box, { paddingLeft: 1, paddingRight: 1 }, autoApproveAll ? (React.createElement(Text, null,
                    React.createElement(Text, { color: "green" }, "\u23F5\u23F5 Auto-approve all enabled"),
                    React.createElement(Text, { color: "gray" }, " (Shift+Tab)"))) : (React.createElement(Text, { color: "gray" }, "Auto-approve all disabled (Shift+Tab)"))))))));
};

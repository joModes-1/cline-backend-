...
function parseInput(text, availableCommands) {
    const highlights = [];
    // Find all mentions
...
export const HighlightedInput = ({ text, cursorPos, availableCommands }) => {
    ...
    if (cursorPos === undefined) {
        ...
        return (React.createElement(Text, null, segments.map((segment, idx) => {
            if (segment.type === "mention" || segment.type === "command") {
                return (React.createElement(Text, { backgroundColor: "gray", key: idx }, segment.text));
            }
    ...
    const renderSegmentWithCursor = (segment, segmentIdx) => {
        ...
        if (safeCursorPos >= segmentStart && safeCursorPos < segmentEnd) {
            ...
            if (isHighlighted) {
                return (React.createElement(Text, { key: segmentIdx },
                    beforeCursor && React.createElement(Text, { backgroundColor: "gray" }, beforeCursor),
                    React.createElement(Text, { backgroundColor: "gray", inverse: true }, cursorChar),
                    afterCursor && React.createElement(Text, { backgroundColor: "gray" }, afterCursor)));
            }
            return (React.createElement(Text, { key: segmentIdx },
                beforeCursor,
        ...
        if (isHighlighted) {
            return (React.createElement(Text, { backgroundColor: "gray", key: segmentIdx }, segment.text));
        }
        return React.createElement(Text, { key: segmentIdx }, segment.text);
    };
    ...
    return (React.createElement(Text, null,
        ...
        cursorAtEnd && React.createElement(Text, { inverse: true }, " ")));
};

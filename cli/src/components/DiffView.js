...
import { computeDiff, getGutterWidth } from "../utils/DiffComputer";
 ...
 */
const DiffLineRow = ({ line, gutterWidth }) => {
    ...
    switch (line.type) {
        case "add":
            return (React.createElement(Box, { flexDirection: "row" },
                React.createElement(Box, { backgroundColor: DIFF_COLORS.addBg, flexShrink: 0 },
                    React.createElement(Text, { color: DIFF_COLORS.gutterFg },
                        lineNumStr,
                        " ")),
                React.createElement(Box, { backgroundColor: DIFF_COLORS.addBg, flexGrow: 1 },
                    React.createElement(Text, { color: DIFF_COLORS.addFg },
                        prefix,
                        line.content))));
        case "remove":
            return (React.createElement(Box, { flexDirection: "row" },
                React.createElement(Box, { backgroundColor: DIFF_COLORS.removeBg, flexShrink: 0 },
                    React.createElement(Text, { color: DIFF_COLORS.gutterFg },
                        lineNumStr,
                        " ")),
                React.createElement(Box, { backgroundColor: DIFF_COLORS.removeBg, flexGrow: 1 },
                    React.createElement(Text, { color: DIFF_COLORS.removeFg },
                        prefix,
                        line.content))));
        case "context":
            return (React.createElement(Box, { flexDirection: "row" },
 ...
 */
const BlockSeparator = () => (React.createElement(Box, { marginY: 0 },
    React.createElement(Text, { color: "gray" }, "─".repeat(40))));
/**
 * Render an ellipsis row for collapsed context lines
 */
const CollapsedRow = ({ count, gutterWidth }) => (React.createElement(Box, { flexDirection: "row" },
    React.createElement(Box, { flexShrink: 0 },
 ...
 */
function collapseContext(block, contextLines) {
 ...
 */
export const DiffView = ({ content, contextLines = DEFAULT_CONTEXT_LINES }) => {
    const diff = useMemo(() => {
        if (!content)
            return null;
    ...
    return (React.createElement(Box, { flexDirection: "column", width: "100%" }, collapsedBlocks.map((displayLines, blockIdx) => (React.createElement(React.Fragment, { key: blockIdx },
        blockIdx > 0 && React.createElement(BlockSeparator, null),
        displayLines.map((item, lineIdx) => item.type === "collapsed" ? (React.createElement(CollapsedRow, { count: item.count, gutterWidth: gutterWidth, key: `${blockIdx}-${lineIdx}` })) : (React.createElement(DiffLineRow, { gutterWidth: gutterWidth, key: `${blockIdx}-${lineIdx}`, line: item.line }))))))));
};

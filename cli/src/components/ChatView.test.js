        ...
        event: (listener) => {
            ...
            return {
                dispose: () => {
                    ...
                    if (idx >= 0)
                        shutdownMockState.listeners.splice(idx, 1);
                },
            };
        },
...
vi.mock("@/core/storage/StateManager", () => ({
    StateManager: {
        get: vi.fn(() => ({
            getGlobalSettingsKey: vi.fn((key) => {
                if (key === "mode")
                    return "act";
...
vi.mock("./ChatMessage", () => ({
    ChatMessage: ({ message }) => React.createElement(Text, null, `Message: ${message?.ts}`),
}));
vi.mock("./FileMentionMenu", () => ({
    FileMentionMenu: () => React.createElement(Text, null, "FileMentionMenu"),
}));
vi.mock("./HighlightedInput", () => ({
    HighlightedInput: ({ text }) => React.createElement(Text, null, `Input: ${text}`),
}));
...
vi.mock("../utils/slash-commands", async (importOriginal) => {
    ...
    return {
        ...
        insertSlashCommand: vi.fn((text) => text),
        sortCommandsWorkflowsFirst: vi.fn((cmds) => cmds),
    };
});
vi.mock("../utils/input", () => ({
...
vi.mock("../utils/parser", () => ({
    jsonParseSafe: vi.fn((_text, defaultValue) => defaultValue),
    parseImagesFromInput: vi.fn((text) => ({ prompt: text, imagePaths: [] })),
}));
vi.mock("../utils/tools", () => ({
...
vi.mock("../utils/cursor", () => ({
    moveCursorUp: vi.fn((_text, pos) => pos),
    moveCursorDown: vi.fn((_text, pos) => pos),
}));
...
describe("ChatView Exit and Cleanup", () => {
    ...
    describe("Initial render state", () => {
        it("should render with input field, footer, and mode toggle visible", () => {
            const { lastFrame } = render(React.createElement(ChatView, { onExit: mockOnExit }));
    ...
    describe("Shutdown event handling", () => {
        it("should subscribe on mount and unsubscribe on unmount", () => {
            const { unmount } = render(React.createElement(ChatView, { onExit: mockOnExit }));
    ...
    describe("Edge cases", () => {
        it("should handle shutdown event when onExit prop is undefined", async () => {
            const { lastFrame } = render(React.createElement(ChatView, null));
            // Fire shutdown event
            shutdownMockState.fire();
            await delay();
...
describe("ChatView UI State During Exit", () => {
    ...
    it("should preserve static content and footer, only hide input during exit", async () => {
        ...
        const { lastFrame } = render(React.createElement(ChatView, { onExit: onExit }));

...
export const SettingsPanelContent = ({ onClose, controller, initialMode, initialModelKey, }) => {
    ...
    const [pickingModelKey, setPickingModelKey] = useState(initialMode ? (initialModelKey ?? "actModelId") : null);
    ...
    // API tab state
    const [separateModels, setSeparateModels] = useState(() => stateManager.getGlobalSettingsKey("planActSeparateModelsSetting") ?? false);
    // Thinking is enabled if budget > 0
    const [actThinkingEnabled, setActThinkingEnabled] = useState(() => (stateManager.getGlobalSettingsKey("actModeThinkingBudgetTokens") ?? 0) > 0);
    const [planThinkingEnabled, setPlanThinkingEnabled] = useState(() => (stateManager.getGlobalSettingsKey("planModeThinkingBudgetTokens") ?? 0) > 0);
    const [actReasoningEffort, setActReasoningEffort] = useState(() => normalizeReasoningEffort(stateManager.getGlobalSettingsKey("actModeReasoningEffort")));
    const [planReasoningEffort, setPlanReasoningEffort] = useState(() => normalizeReasoningEffort(stateManager.getGlobalSettingsKey("planModeReasoningEffort")));
    ...
    // Other tab state
    const [preferredLanguage, setPreferredLanguage] = useState(() => stateManager.getGlobalSettingsKey("preferredLanguage") || "English");
    const [telemetry, setTelemetry] = useState(() => stateManager.getGlobalSettingsKey("telemetrySetting") || "unset");
    // Account tab state
    const [accountEmail, setAccountEmail] = useState(null);
    const [accountBalance, setAccountBalance] = useState(null);
    const [accountOrganization, setAccountOrganization] = useState(null);
    const [accountOrganizations, setAccountOrganizations] = useState(null);
    ...
    const [provider, setProvider] = useState(() => stateManager.getApiConfiguration().actModeApiProvider ||
    ...
    const { isWaiting: isWaitingForOcaAuth, startAuth: startOcaAuth, cancelAuth: cancelOcaAuth, isAuthenticated: isOcaAuthenticated, } = useOcaAuth({
        controller,
    ...
    const { actModelId, planModelId } = useMemo(() => {
        ...
        return {
            actModelId: actKey ? stateManager.getGlobalSettingsKey(actKey) || "" : "",
            planModelId: planKey ? stateManager.getGlobalSettingsKey(planKey) || "" : "",
        };
    ...
    const handleOrganizationSelect = useCallback(async (orgId) => {
        if (!controller) {
            return;
        }
        ...
        switch (currentTab) {
            case "api":
                return [
                    ...
                    ...(provider === "cline"
                        ? [{ key: "viewAccount", label: "View account", type: "action", value: "" }]
                        : []),
                    ...(separateModels
                        ? [
                            { key: "spacer0", label: "", type: "spacer", value: "" },
                            { key: "actHeader", label: "Act Mode", type: "header", value: "" },
                            {
                                key: "actModelId",
                                label: "Model ID",
                                type: "editable",
                            ...
                            ...(showActReasoningEffort
                                ...
                                : []),
                            { key: "planHeader", label: "Plan Mode", type: "header", value: "" },
                            {
                                key: "planModelId",
                                label: "Model ID",
                                type: "editable",
                            ...
                            ...(showPlanReasoningEffort
                                ...
                                : []),
                            { key: "spacer1", label: "", type: "spacer", value: "" },
                        ]
                        : [
                            {
                                key: "actModelId",
            ...
            case "auto-approve": {
                ...
                const addActionPair = (parentKey, parentLabel, parentDesc, childKey, childLabel, childDesc) => {
                    result.push({
                        key: parentKey,
                        label: parentLabel,
                        type: "checkbox",
                        value: actions[parentKey] ?? false,
                ...
                addActionPair("readFiles", "Read project files", "Read files in the working directory", "readFilesExternally", "Read all files", "Read files outside working directory");
                addActionPair("editFiles", "Edit project files", "Edit files in the working directory", "editFilesExternally", "Edit all files", "Edit files outside working directory");
                addActionPair("executeSafeCommands", "Execute safe commands", "Run low-risk terminal commands", "executeAllCommands", "Execute all commands", "Run any terminal command");
                result.push({
                    key: "useBrowser",
    ...
    const setReasoningEffortForMode = useCallback((mode, effort) => {
        if (mode === "act") {
    ...
    const handleAction = useCallback(() => {
        ...
        if (item.key === "separateModels") {
            ...
            if (!newValue) {
                ...
                if (actProvider) {
                    ...
                    if (planKey)
                        stateManager.setGlobalState(planKey, actModel);
                }
        ...
        if (item.key === "telemetry") {
            const newTelemetry = newValue ? "enabled" : "disabled";
    ...
    const handleBedrockCustomFlowComplete = useCallback(async (arn, baseModelId) => {
        if (!pickingModelKey)
            return;
        const apiConfig = stateManager.getApiConfiguration();
    ...
    const handleProviderSelect = useCallback(async (providerId) => {
        ...
        if (keyField) {
            ...
            const existingKey = apiConfig[fieldName] || "";
            setPendingProvider(providerId);
    ...
    const handleApiKeySubmit = useCallback(async (submittedValue) => {
    ...
    const handleBedrockComplete = useCallback((bedrockConfig) => {
    ...
    const navigateItems = useCallback((direction) => {
        setSelectedIndex((i) => {
    ...
    const renderContent = () => {
        if (isPickingProvider) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { bold: true, color: COLORS.primaryBlue }, "Select Provider"),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(ProviderPicker, { isActive: isPickingProvider, onSelect: handleProviderSelect })),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Type to search, arrows to navigate, Enter to select, Esc to cancel"))));
        }
        if (isEnteringApiKey && pendingProvider) {
            return (React.createElement(ApiKeyInput, { isActive: isEnteringApiKey, onCancel: () => {
                    setIsEnteringApiKey(false);
                    setPendingProvider(null);
                    setApiKeyValue("");
                }, onChange: setApiKeyValue, onSubmit: handleApiKeySubmit, providerName: getProviderLabel(pendingProvider), value: apiKeyValue }));
        }
        if (isConfiguringBedrock) {
            return (React.createElement(BedrockSetup, { isActive: isConfiguringBedrock, onCancel: () => {
                    setIsConfiguringBedrock(false);
                    setPendingProvider(null);
        ...
        if (codexAuthError) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { bold: true, color: "red" }, "ChatGPT sign-in failed"),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "yellow" }, codexAuthError)),
                React.createElement(Box, { marginTop: 1 },
        ...
        if (isPickingFeaturedModel && pickingModelKey) {
            ...
            return (React.createElement(FeaturedModelPicker, { featuredModels: featuredModels, helpText: "Arrows to navigate, Enter to select, Esc to cancel", selectedIndex: featuredModelIndex, title: `Select: ${label}` }));
        }
        if (isPickingModel && pickingModelKey) {
            ...
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { bold: true, color: COLORS.primaryBlue },
                    ...
                    label),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(ModelPicker, { controller: controller, isActive: isPickingModel, onChange: () => { }, onSubmit: handleModelSelect, provider: provider })),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Type to search, arrows to navigate, Enter to select, Esc to cancel"))));
        }
        if (isPickingLanguage) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { bold: true, color: COLORS.primaryBlue }, "Select Language"),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(LanguagePicker, { isActive: isPickingLanguage, onSelect: handleLanguageSelect })),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Type to search, arrows to navigate, Enter to select, Esc to cancel"))));
        }
        if (isPickingOrganization && accountOrganizations) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { bold: true, color: COLORS.primaryBlue }, "Select Organization"),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(OrganizationPicker, { isActive: isPickingOrganization, onSelect: handleOrganizationSelect, organizations: accountOrganizations })),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Arrows to navigate, Enter to select, Esc to cancel"))));
        }
        if (isWaitingForClineAuth) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Box, null,
                    React.createElement(Text, { color: COLORS.primaryBlue },
                        React.createElement(Spinner, { type: "dots" })),
                    React.createElement(Text, { color: "white" }, " Waiting for Cline sign-in...")),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Complete sign-in in your browser.")),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Esc to cancel"))));
        }
        if (isShowingOcaEmployeeCheck) {
            return (React.createElement(OcaEmployeeCheck, { isActive: isShowingOcaEmployeeCheck, onCancel: () => setIsShowingOcaEmployeeCheck(false), onSignIn: () => {
        ...
        if (isWaitingForOcaAuth) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Box, null,
                    React.createElement(Text, { color: COLORS.primaryBlue },
                        React.createElement(Spinner, { type: "dots" })),
                    React.createElement(Text, { color: "white" }, " Waiting for OCA sign-in...")),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Complete sign-in in your browser.")),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, "Esc to cancel"))));
        ...
        if (isBedrockCustomFlow) {
            return (React.createElement(BedrockCustomModelFlow, { isActive: isBedrockCustomFlow, onCancel: () => {
                    setIsBedrockCustomFlow(false);
                    setIsPickingModel(true);
        ...
        if (currentTab === "account" && !accountEmail && !isAccountLoading) {
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { color: "white" }, "Sign in to access Cline features:"),
                React.createElement(Box, { flexDirection: "column", marginTop: 1 },
                    React.createElement(Text, { color: "gray" }, " - Free access to frontier AI models"),
                    React.createElement(Text, { color: "gray" }, " - Built-in web search capabilities"),
                    React.createElement(Text, { color: "gray" }, " - Team management and shared billing")),
                React.createElement(Box, { marginTop: 1 }, items.map((item, idx) => {
                    const isSelected = idx === selectedIndex;
                    return (React.createElement(Text, { key: item.key },
                        React.createElement(Text, { bold: true, color: isSelected ? COLORS.primaryBlue : undefined },
                            isSelected ? "❯" : " ",
                            " "),
                        React.createElement(Text, { color: isSelected ? COLORS.primaryBlue : "white" }, item.label),
                        isSelected && React.createElement(Text, { color: "gray" }, " (Enter)")));
                }))));
        }
        if (isEditing) {
            const item = items[selectedIndex];
            return (React.createElement(Box, { flexDirection: "column" },
                React.createElement(Text, { bold: true, color: COLORS.primaryBlue },
                    ...
                    item?.label),
                React.createElement(Box, { marginTop: 1 },
                    React.createElement(Text, { color: "white" }, editValue),
        ...
        return (React.createElement(Box, { flexDirection: "column" }, items.map((item, idx) => {
            const isSelected = idx === selectedIndex;
            if (item.type === "header") {
                return (React.createElement(Box, { key: item.key, marginTop: idx > 0 ? 0 : 0 },
                    React.createElement(Text, { bold: true, color: "white" }, item.label)));
            }
            if (item.type === "spacer") {
                return React.createElement(Box, { key: item.key, marginTop: 1 });
            }
            if (item.type === "separator") {
                return (React.createElement(Box, { borderBottom: false, borderColor: "gray", borderDimColor: true, borderLeft: false, borderRight: false, borderStyle: "single", borderTop: true, key: item.key, width: "100%" }));
            }
            if (item.type === "checkbox") {
                return (React.createElement(Box, { key: item.key, marginLeft: item.isSubItem ? 2 : 0 },
                    React.createElement(Checkbox, { checked: Boolean(item.value), description: item.description, isSelected: isSelected, label: item.label })));
            }
            ...
            if (item.type === "cycle") {
                return (React.createElement(Text, { key: item.key },
                    ...
                    React.createElement(Text, { color: isSelected ? COLORS.primaryBlue : "white" },
                        item.label,
                        ": "),
                    React.createElement(Text, { color: COLORS.primaryBlue }, typeof item.value === "string" ? item.value : String(item.value)),
                    isSelected && React.createElement(Text, { color: "gray" }, " (Tab to cycle)")));
            }
            ...
            return (React.createElement(Text, { key: item.key },
                React.createElement(Text, { bold: true, color: isSelected ? COLORS.primaryBlue : undefined },
                    isSelected ? "❯" : " ",
                    " "),
                item.label && React.createElement(Text, { color: isSelected ? COLORS.primaryBlue : "white" },
                    item.label,
                    ": "),
                React.createElement(Text, { color: item.type === "readonly" ? "gray" : COLORS.primaryBlue }, typeof item.value === "string" ? item.value : String(item.value)),
                item.type === "editable" && isSelected && React.createElement(Text, { color: "gray" }, " (Tab to edit)")));
        })));
    };
    // Determine if we're in a subpage (picker, editor, or waiting state)
    const isSubpage = isPickingProvider ||
        isPickingModel ||
        ...
        isBedrockCustomFlow ||
        isEditing;
    return (React.createElement(Panel, { currentTab: currentTab, isSubpage: isSubpage, label: "Settings", tabs: TABS }, renderContent()));
};
//# sourceMappingURL=SettingsPanelContent.js.map
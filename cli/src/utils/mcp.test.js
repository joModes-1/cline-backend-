...
async function readMcpSettings(configDir) {
...
describe("addMcpServerShortcut", () => {
    ...
    it("errors when URL is provided without --type http", async () => {
        const configDir = await createTempConfigDir();
        await expect(addMcpServerShortcut("linear", ["https://mcp.linear.app/mcp"], { config: configDir })).rejects.toThrow("Use --type http");
    });
});

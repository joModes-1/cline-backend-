...
describe("display", () => {
    ...
    describe("formatMessage", () => {
        ...
        describe("say messages", () => {
            ...
            it("should show unknown say types in verbose mode", () => {
                const message = createMessage({ say: "unknown_type", text: "test" });
    ...
    describe("formatState", () => {
        it("should format state with messages", () => {
            const state = {
                clineMessages: [{ ts: Date.now(), type: "say", say: "text", text: "Hello" }],
            };
    ...
    describe("Spinner", () => {
        ...
        it("should start spinning with message", () => {
            ...
            expect(calls.some((c) => typeof c === "string" && c.includes("Loading..."))).toBe(true);
        });
        it("should update message", () => {
            ...
            vi.advanceTimersByTime(80);
            const calls = writeSpy.mock.calls.map((c) => c[0]);
            expect(calls.some((c) => typeof c === "string" && c.includes("Updated"))).toBe(true);
        });
        it("should stop with final message", () => {
            spinner.start("Loading...");
            spinner.stop("Done!");
            const calls = writeSpy.mock.calls.map((c) => c[0]);
            expect(calls.some((c) => typeof c === "string" && c.includes("Done!"))).toBe(true);
        });
        ...
        it("should show failure message", () => {
            ...
            spinner.fail("Failed!");
            const calls = writeSpy.mock.calls.map((c) => c[0]);
            expect(calls.some((c) => typeof c === "string" && c.includes("Failed!"))).toBe(true);
        });
    });
});

 ...
 */
function parseSearchReplacePairs(content) {
    const pairs = [];
...
export function getGutterWidth(diff) {
    ...
    return Math.max(1, maxLineNum.toString().length);
            ...
            const replaceContent = replaceEndIndex !== -1
 ...
 */
function parseApplyPatchPairs(content) {
    const pairs = [];
 ...
 */
function computeLineDiff(search, replace) {
    const lines = [];
    let additions = 0;
    let deletions = 0;
    // Handle empty cases

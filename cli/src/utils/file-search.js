...
function addParentDirs(relativePath, dirSet) {
...
function dirsToResults(dirSet) {
    return Array.from(dirSet, (p) => ({ path: p, type: "folder", label: basename(p) }));
}
async function listFilesWithNodeFs(workspacePath, limit) {
    const files = [];
    const dirs = new Set();
...
async function listFilesWithRipgrep(workspacePath, limit) {
...
export function checkAndWarnRipgrepMissing() {
...
export function getRipgrepInstallInstructions() {
...
export async function listWorkspaceFiles(workspacePath, limit = 5000) {
    if (checkRipgrep()) {
        try {
...
const orderByMatchScore = (a, b) => countGaps(a.positions) - countGaps(b.positions);
export async function searchWorkspaceFiles(query, workspacePath, limit = 15, selectedType) {
    try {
...
export function extractMentionQuery(text) {
...
export function insertMention(text, atIndex, filePath) {

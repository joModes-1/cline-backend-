...
function parseMcpSettings(content, settingsPath) {
...
function createMcpServerConfig(targetOrCommand, transportType) {
        ...
        if (targetOrCommand.length === 1) {
            ...
            try {
                ...
                if (parsedUrl.protocol === "http:" || parsedUrl.protocol === "https:") {
                    throw new Error(`Looks like you provided a URL for '${value}'. Use --type http, for example: cline mcp add <name> ${value} --type http`);
                }
            }
            catch (error) {
    ...
    if (targetOrCommand.length !== 1) {
        throw new Error("HTTP/SSE MCP servers require exactly one URL. Example: cline mcp add linear https://mcp.linear.app/mcp --type http");
    }
    const config = {
...
export async function addMcpServerShortcut(name, targetOrCommand = [], options) {

 ...
 */
function createClineMessage(overrides) {
    return {
        ts: Date.now(),
        type: "say",
        ...overrides,
 ...
 */
const VALID_TOOL_CALL_STATUSES = ["pending", "in_progress", "completed", "failed"];
/**
 * Valid PlanEntryStatus values per ACP schema.
 */
const VALID_PLAN_ENTRY_STATUSES = ["pending", "in_progress", "completed"];
/**
 * Valid PlanEntryPriority values per ACP schema.
 */
const VALID_PLAN_ENTRY_PRIORITIES = ["high", "medium", "low"];
/**
 ...
 */
function assertValidToolCallStatus(status) {
    if (status !== undefined) {
 ...
 */
function assertValidToolCall(update) {
 ...
 */
function assertValidToolCallUpdate(update) {
 ...
 */
function assertValidPlanEntry(entry) {
 ...
 */
function assertValidContentBlock(content) {
...
describe("createSessionState", () => {
    ...
    it("should create independent state objects", () => {
        ...
        state1.pendingToolCalls.set("tool-1", {});
...
describe("translateMessage - say messages", () => {
    ...
    describe("text messages", () => {
        it("should translate say:text to agent_message_chunk", () => {
            ...
            const chunk = result.updates[0];
            assertValidContentBlock(chunk.content);
            expect(chunk.content.type).toBe("text");
            expect(chunk.content.text).toBe("Hello, this is a response.");
        });
    ...
    describe("reasoning messages", () => {
        it("should translate say:reasoning to agent_thought_chunk", () => {
            ...
            assertValidContentBlock(chunk.content);
            expect(chunk.content.type).toBe("text");
            expect(chunk.content.text).toBe("I need to analyze the code structure first.");
        });
        it("should fall back to text field if reasoning is undefined", () => {
            ...
            expect(result.updates[0].sessionUpdate).toBe("agent_thought_chunk");
            const chunk = result.updates[0];
            expect(chunk.content.text).toBe("Thinking about the problem...");
        });
    });
    describe("error messages", () => {
        it("should translate say:error to agent_message_chunk with error prefix", () => {
            ...
            expect(chunk.content.text).toBe("Error: Failed to read file");
        });
...
- Working on step 3`,
    ...
    describe("hook status messages", () => {
        it("should format hook_status as human-readable message", () => {
            ...
            expect(messageChunk).toBeDefined();
            const chunk = messageChunk;
            expect(chunk.content.text).toContain("pre-commit");
            // The status is formatted with capital R: "Running..."
            expect(chunk.content.text.toLowerCase()).toContain("running");
        });
...
describe("translateMessage - ask messages", () => {
    ...
    describe("followup questions", () => {
        it("should translate ask:followup to agent_message_chunk", () => {
            ...
            expect(messageChunk).toBeDefined();
            const chunk = messageChunk;
            expect(chunk.content.text).toBe("What would you like me to do next?");
        ...
        it("should handle plain text followup", () => {
            ...
            const messageChunk = result.updates.find((u) => u.sessionUpdate === "agent_message_chunk");
            const chunk = messageChunk;
            expect(chunk.content.text).toBe("Do you want to continue?");
        });
    });
    describe("plan mode respond", () => {
        it("should translate ask:plan_mode_respond to agent_message_chunk", () => {
            ...
            expect(messageChunk).toBeDefined();
            const chunk = messageChunk;
            expect(chunk.content.text).toBe("Here is my plan for the task...");
        });
    });
...
//# sourceMappingURL=messageTranslator.test.js.map
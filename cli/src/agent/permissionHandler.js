 ...
 */
const STANDARD_PERMISSION_OPTIONS = [
 ...
 */
const RESTRICTED_PERMISSION_OPTIONS = [
    { kind: "allow_once", optionId: "allow_once", name: "Allow Once" },
    { kind: "reject_once", optionId: "reject_once", name: "Reject" },
];
/**
 ...
 */
const PERMISSION_REQUIRING_ASK_TYPES = new Set([
    "command",
    "tool",
 ...
 */
export function requiresPermission(askType) {
 ...
 */
export function getPermissionOptionsForAskType(askType) {
 ...
 */
export function handlePermissionResponse(response, askType) {
    // Check if cancelled
 ...
 */
export function createPermissionRequest(toolCall, askType) {
...
export class AutoApprovalTracker {
    ...
    autoApprovedMcpServers = new Set();
    /**
     ...
     */
    recordAlwaysAllow(askType, identifier) {
        switch (askType) {
            case "command":
     ...
     */
    isAutoApproved(askType, identifier) {
        switch (askType) {
            case "command":
                const commandPrefix = identifier.split(" ")[0];
 ...
 */
export async function processPermissionRequest(requestPermission, sessionId, toolCall, askType, identifier, autoApprovalTracker) {
 ...
 */
export function getAutoApprovalIdentifier(toolCall, askType) {
    const rawInput = toolCall.rawInput;
    switch (askType) {
        case "command":
            return rawInput?.command || toolCall.title;
        case "tool":
            // Try to get tool name from raw input or title
            return rawInput?.tool || toolCall.title;
        case "use_mcp_server":
            return rawInput?.serverName || toolCall.title;
        default:
 ...
 */
export function updateSessionStateAfterPermission(sessionState, toolCallId, approved) {

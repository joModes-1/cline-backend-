...
import { anthropicDefaultModelId, anthropicModels, bedrockDefaultModelId, bedrockModels, deepSeekDefaultModelId, deepSeekModels, geminiDefaultModelId, geminiModels, groqDefaultModelId, groqModels, mistralDefaultModelId, mistralModels, moonshotDefaultModelId, moonshotModels, openAiCodexDefaultModelId, openAiNativeDefaultModelId, openAiNativeModels, xaiDefaultModelId, xaiModels, } from "@shared/api";
...
function hasStaticModels(provider) {
...
export class ClineAgent {
    ...
    #sessionControllers = new WeakMap();
     ...
     */
    async initialize(params, connection) {
     ...
     */
    initializeHostProvider(clientCapabilities, connection) {
        const hostBridgeClientProvider = new ACPHostBridgeClientProvider(clientCapabilities, () => this.currentActiveSessionId, () => this.sessions.get(this.currentActiveSessionId ?? "")?.cwd ?? process.cwd(), AGENT_VERSION);
        HostProvider.initialize(() => new CliWebviewProvider(this.ctx.extensionContext), () => {
        ...
        }, hostBridgeClientProvider, (message) => Logger.info(message), async (path) => {
            return AuthHandler.getInstance().getCallbackUrl(path);
        }, async () => "", // get binary location not needed in ACP mode
        this.ctx.EXTENSION_DIR, this.ctx.DATA_DIR);
    }
    /**
     ...
     */
    async getSessionModelState(mode) {
        ...
        const currentFullModelId = currentProvider && currentModelId ? `${currentProvider}/${currentModelId}` : currentProvider || "";
        ...
        const availableModels = modelIds.map((modelId) => ({
    ...
    async unstable_setSessionModel(params) {
        ...
        const provider = params.modelId.substring(0, slashIndex);
    ...
    async prompt(params) {
        ...
        try {
            ...
            if (controller.task) {
                const onClineMessagesChanged = (change) => {
                    this.handleClineMessagesChanged(params.sessionId, sessionState, change, resolvePrompt, promptResolved).catch((error) => {
    ...
    async handleClineMessagesChanged(sessionId, sessionState, change, resolvePrompt, promptResolved) {
     ...
     */
    async handlePermissionRequest(sessionId, sessionState, message, permissionRequest) {
     ...
     */
    checkMessageForPromptResolution(message, resolvePrompt, promptResolved) {
        if (promptResolved.value)
            return;
     ...
     */
    async processMessageWithDelta(sessionId, sessionState, message) {
        ...
        const isTextStreamingMessage = (message.type === "say" &&
        ...
        if (isTextStreamingMessage && message.text) {
            ...
            if (textDelta) {
                ...
                const sessionUpdate = message.type === "say" && message.say === "reasoning" ? "agent_thought_chunk" : "agent_message_chunk";
                ...
                const isCompletionResult = (message.type === "say" && message.say === "completion_result") ||
    ...
    async cancel(params) {
        ...
        if (sessionState) {
            ...
            // Check if we have an active controller task, cancel it
            const controller = this.#sessionControllers.get(session);
            if (controller?.task) {
                try {
    ...
    async authenticate(params) {
        ...
        if (!this.webviewProvider) {
            this.webviewProvider = new CliWebviewProvider(this.ctx.extensionContext);
        }
        try {
     ...
     */
    async emitSessionUpdate(sessionId, update) {
     ...
     */
    async sendAvailableCommands(sessionId, controller) {
        try {
            ...
            const filteredCommands = response.commands.filter((cmd) => cmd.cliCompatible && !cliOnlyNames.has(cmd.name) && !vscodeOnlyNames.has(cmd.name));
            // Convert to ACP AvailableCommand format
            const availableCommands = filteredCommands.map((cmd) => ({
                name: cmd.name,
    ...
    async authenticateOpenAiCodex() {
        ...
        try {
            ...
            if (actModelKey)
                stateManager.setGlobalState(actModelKey, openAiCodexDefaultModelId);
            if (planModelKey)
                stateManager.setGlobalState(planModelKey, openAiCodexDefaultModelId);
            await stateManager.flushPendingState();
            return {};
        }
        catch (error) {
...
//# sourceMappingURL=ClineAgent.js.map
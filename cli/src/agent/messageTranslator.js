 ...
 */
export function translateMessage(message, sessionState, options) {
    const updates = [];
 ...
 */
function translateSayMessage(message, sessionState, _options) {
    const updates = [];
    let toolCallId;
    ...
    switch (say) {
        ...
        case "hook_status":
            ...
            if (message.text) {
                try {
                    const hookInfo = JSON.parse(message.text);
 ...
 */
function translateAskMessage(message, sessionState, options) {
    const updates = [];
 ...
 */
function translateToolMessage(message, sessionState) {
    const updates = [];
    if (!message.text)
        return updates;
    try {
        ...
        const status = message.partial ? "in_progress" : "completed";
        // Build title
 ...
 */
function translateCommandMessage(message, sessionState) {
    const updates = [];
 ...
 */
function translateCommandOutputMessage(message, sessionState) {
    const updates = [];
    if (sessionState.currentToolCallId) {
        const status = message.commandCompleted ? "completed" : "in_progress";
        updates.push({
            sessionUpdate: "tool_call_update",
            toolCallId: sessionState.currentToolCallId,
            status,
 ...
 */
function translateBrowserActionMessage(message, sessionState) {
    const updates = [];
    try {
        const action = message.text ? JSON.parse(message.text) : null;
        const toolCallId = sessionState.currentToolCallId || generateToolCallId();
        if (!sessionState.currentToolCallId) {
            sessionState.currentToolCallId = toolCallId;
        }
 ...
 */
function translateMcpMessage(message, sessionState) {
    const updates = [];
    try {
 ...
 */
function translateTaskProgressMessage(message) {
    const updates = [];
    if (!message.text)
        return updates;
 ...
 */
function parseTaskProgressToEntries(text) {
    const entries = [];
 ...
 */
function buildToolTitle(toolInfo) {
 ...
 */
export function translateMessages(messages, sessionState) {
    const allUpdates = [];
